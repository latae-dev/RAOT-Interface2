...
// include model
require_once '../../models/parameter/wrd_fund_model.php';
...
class WrdFundController {
    ...
    public function __construct() {
        ...
        $this->model = new WrdFundModel($db);
    }
    ...
    private function getWrdFunds() {
        try {
            $data = $this->model->readAll();
            if ($data === false) {
                $this->sendResponse(['status' => 'error', 'message' => 'Failed to fetch WRD funds'], 500);
                return;
            }
            $this->sendResponse(['status' => 'success', 'data' => $data]);
        } catch (Exception $e) {
            error_log("Error in getWrdFunds: " . $e->getMessage());
            $this->sendResponse(['status' => 'error', 'message' => 'Internal server error'], 500);
        }
    }

    private function getWrdFund() {
        $id = $_GET['id'] ?? '';
        if (empty($id)) {
        ...
        try {
            ...
            if ($data === false) {
                $this->sendResponse(['status' => 'error', 'message' => 'WRD fund not found'], 404);
                return;
            }
            $this->sendResponse(['status' => 'success', 'data' => $data]);
        } catch (Exception $e) {
            error_log("Error in getWrdFund: " . $e->getMessage());
            $this->sendResponse(['status' => 'error', 'message' => 'Internal server error'], 500);
        }
    }
...
$controller = new WrdFundController();
