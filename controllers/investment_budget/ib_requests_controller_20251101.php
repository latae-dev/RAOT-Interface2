...
class IbRequestsController
{
    ...
    private function getApprovalRequests()
    {
        ...
        // error_log('  - depart_id: ' . ($context['depart_id'] ?? 'null'));
    ...
    private function getCurrentUserContext()
    {
        ...
        $context = [
            ...
            'depart_id' => $_SESSION['depart_id'] ?? null,  // เพิ่ม depart_id
    ...
    private function buildApprovalScope(array $context)
    {
        ...
        if ($this->isTruthy($context['approve_branch'] ?? null)) {
            ...
            // เพิ่ม depart_id filter สำหรับ branch level
            $departValue = $this->normalizeScopeValue($context['depart_id'] ?? null);
            if ($departValue !== null && $departValue !== '') {
                $scope['depart_ids'][] = $departValue;
            } else {
                $scope['depart_ids'][] = '*';
            }
        }
    ...
    {
        // ถ้าไม่มี scope หรือไม่มี depart_ids ให้ผ่านไปเลย
        ...
        // ถ้า depart_ids ว่างเปล่า หรือมี wildcard ให้ผ่าน
        if (empty($departIds) || in_array('*', $departIds, true)) {
            return true;
        }

        // ดึง department_id จาก request (มาจาก JOIN users.tb_users)
        $requestDepartId = $this->normalizeScopeValue($request['department_id'] ?? null);

        // ถ้าคำขอไม่มี department_id ให้ไม่ผ่าน (เพื่อความปลอดภัย)
    ...
    private function evaluateApprovalScope(?array $scope, array $request)
    {
        ...
        if (in_array('*', $ids, true)) {
            // ตรวจสอบ depart_id สำหรับ branch level
    ...
    /**
     * หา stage levels ที่ user สามารถเห็นได้
     * - เห็น stage ที่ตัวเองอนุมัติ
     * - เห็น stage ที่ผ่านไปแล้ว (ระดับที่สูงกว่า)
     * - ไม่เห็น stage ที่ยังไม่ถึงคิว (ระดับที่ต่ำกว่า)
     */
    private function getUserVisibleStages(array $context)
    {
        $visibleStages = [];

        // Map approval permission to stage name and level
        $stageMap = [
        ...
        // หา level สูงสุดของ user
        foreach ($stageMap as $stage => $info) {
            $hasPermission = $this->isTruthy($context[$info['permission']] ?? null);

            // สำหรับ head_office ต้องมี approve_investment ด้วย
            if ($stage === 'head_office') {
                $hasPermission = $hasPermission && $this->isTruthy($context['approve_investment'] ?? null);
            }

            if ($hasPermission && $info['level'] > $userLevel) {
        ...
        // เห็น stage ของตัวเอง + stage ที่สูงกว่า (ผ่านไปแล้ว)
        foreach ($stageMap as $stage => $info) {
