...
include_once __DIR__ . '/ib_approval_hierarchy.php';
...
class IbRequestsController
{
    ...
    public function __construct()
    {
        ...
        $this->approvalHierarchy = new IbApprovalHierarchy($this->ibRequestsModel, $db);
    }
        ...
        switch ($method) {
            case 'GET':
                if (isset($_GET['action'])) {
                    if ($_GET['action'] == 'all') {
                        ...
                        $this->getMyRequests();
                    } elseif ($_GET['action'] == 'user_hierarchy') {
                        $this->getUserHierarchy();
                    } elseif ($_GET['action'] == 'asset_numbers') {
            ...
            case 'POST':
                if (isset($_GET['action'])) {
                    ...
                    } elseif ($_GET['action'] == 'budget_approve' && isset($_GET['id'])) {
                        $this->budgetApprove($_GET['id']);
                    } else {
    ...
    private function getAllRequests()
    {
        ...
        if ($result !== false) {
            ...
            $error = method_exists($this->ibRequestsModel, 'getLastError')
                ? $this->ibRequestsModel->getLastError()
                : '';
            $this->responseError('Failed to retrieve requests ' . $error, 500);
        }
    }
    ...
    /**
     * ดึงข้อมูล user hierarchy ของผู้ใช้ปัจจุบัน
     * สำหรับแสดงใน UI (ระดับการอนุมัติ, รหัสแผนก, ฯลฯ)
     */
    private function getUserHierarchy()
    {
        // ดึง context ของ user ปัจจุบัน
        $context = $this->approvalHierarchy->getCurrentUserContext();

        if (!$context || empty($context['user_id'])) {
            $this->responseError('User not authenticated', 401);
            return;
        }

        // ดึงข้อมูล hierarchy แบบเต็ม
        $hierarchyInfo = $this->approvalHierarchy->getDebugInfo($context);

        // เพิ่ม mapping ระหว่าง approval_level กับ version_plan_id (ใช้ ID จากตาราง tb_ib_version_plan)
        ...
        $approvalLevel = $hierarchyInfo['approval_level'] ?? null;
        $detectedLevel = $hierarchyInfo['depart_detected_level'] ?? null;

        $allowedVersionPlanId = null;
        if ($approvalLevel && isset($levelToVersionMapping[$approvalLevel])) {
            $allowedVersionPlanId = $levelToVersionMapping[$approvalLevel];
        } elseif ($detectedLevel && isset($levelToVersionMapping[$detectedLevel])) {
            $allowedVersionPlanId = $levelToVersionMapping[$detectedLevel];
        }

        $hierarchyInfo['allowed_version_plan_id'] = $allowedVersionPlanId;

        // ส่งกลับข้อมูล
        $this->responseSuccess('User hierarchy retrieved successfully', $hierarchyInfo);
    }

    /**
     * ดึงข้อมูลหมายเลขครุภัณฑ์ทั้งหมด
     * สำหรับใช้ใน dropdown เลือกหมายเลขครุภัณฑ์ทดแทน
     */
    private function getAssetNumbers()
    {
        try {
            $database = new Database();
            $db = $database->connect('raot_db_qas');
            ...
            if (!$db) {
                $this->responseError('Database connection failed', 500);
                return;
            }

            // Query ข้อมูลจาก parameters.tb_ib_asset_numbers
            $query = "SELECT id, code, name, is_active
                      FROM parameters.tb_ib_asset_numbers
                      WHERE is_active = 1
                      ORDER BY code ASC";
            ...
            if (function_exists('pg_query')) {
                ...
                if (!$result) {
                    $this->responseError('Query failed: ' . pg_last_error($db), 500);
                    return;
                }
                ...
                // PDO fallback
                $stmt = $db->query($query);
                $assetNumbers = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Convert to consistent format
                $assetNumbers = array_map(function ($row) {
                    return [
                        'id' => (int)$row['id'],
                        'code' => $row['code'],
                        'name' => $row['name'],
            ...
            $this->responseSuccess('Asset numbers retrieved successfully', $assetNumbers);
        } catch (Exception $e) {
            $this->responseError('Error retrieving asset numbers: ' . $e->getMessage(), 500);
        }
    }
    ...
    /**
     * บันทึกผลการอนุมัติงบประมาณ
     */
    private function budgetApprove($id)
    {
        $payload = $_POST;
        ...
        if (!in_array($action, ['approved', 'rejected'], true)) {
            $this->responseError('invalid action', 400);
            return;
        }

        $request = $this->ibRequestsModel->readRequest($id);
        if (!$request) {
            $this->responseError('ไม่พบคำขอ', 404);
            return;
        }
        ...
        $context = $this->getCurrentUserContext();
        $canApproveBudget = $context
            && $this->isTruthy($context['approve_head_office'] ?? null)
            && $this->isTruthy($context['approve_investment'] ?? null);

        if (!$currentUserId || !$canApproveBudget) {
            $this->responseError('ไม่มีสิทธิ์อนุมัติงบประมาณ', 403);
            return;
        }

        // ไม่ให้บันทึกซ้ำเฉพาะกรณีมีผลพิจารณาสุดท้ายแล้ว (approved/rejected)
        $decisionStatus = strtolower(trim((string)($request['budget_approval_status'] ?? '')));
        if (in_array($decisionStatus, ['approved', 'rejected'], true)) {
            $this->responseError('รายการนี้มีผลพิจารณางบประมาณแล้ว', 400);
            return;
        }

        $amountValue = $amount !== null && $amount !== '' ? (float)$amount : null;

        $saved = $this->ibRequestsModel->saveBudgetApproval(
            $id,
            $action,
            $amountValue,
            $remark,
        ...
        if ($saved) {
            $this->responseSuccess('บันทึกผลอนุมัติงบประมาณสำเร็จ');
        } else {
            $this->responseError('บันทึกไม่สำเร็จ', 500);
        }
    }
    ...
    private function getApprovalRequests()
    {
        ...
        // หา status ที่ user เห็นได้ตามระดับการอนุมัติ
        $visibleStatuses = $this->getUserVisibleStages($context);

        // Debug: Log approval scope
        // error_log('🔍 User ' . ($context['user_id'] ?? 'unknown') . ' approval context:');
        // error_log('  - depart_code: ' . ($context['depart_code'] ?? 'null'));
        ...
        // error_log('👁️ Visible statuses: ' . json_encode($visibleStatuses));

        $filters = [];

        $status = isset($_GET['status']) ? strtolower(trim($_GET['status'])) : '';
        if ($status === '' || $status === null || $status === 'submitted') {
            // ใช้ visible_statuses แทน status_prefix
            if (!empty($visibleStatuses)) {
                $filters['visible_statuses'] = $visibleStatuses;
            } else {
                $filters['status_prefix'] = 'pending_';
            }
        } elseif ($status !== 'all') {
        ...
        if ($result !== false) {
            ...
            // เพิ่มข้อมูล debug ถ้ามี parameter debug=true (สำหรับแสดง user hierarchy ที่ UI)
            if (!empty($_GET['debug']) && $_GET['debug'] === 'true') {
                $debugInfo = $this->approvalHierarchy->getDebugInfo($context);

                // เพิ่ม debug_info ใน meta
                if (isset($result['meta'])) {
                    $result['meta']['debug_info'] = $debugInfo;
                }
            }
    ...
    /**
     * ดึงข้อมูล user context จาก session
     * เรียกใช้จาก IbApprovalHierarchy
     */
    private function getCurrentUserContext()
    {
        return $this->approvalHierarchy->getCurrentUserContext();
    }

    /**
     * สร้าง approval scope จาก user context
     * เรียกใช้จาก IbApprovalHierarchy
     */
    private function buildApprovalScope(array $context)
    {
        return $this->approvalHierarchy->buildApprovalScope($context);
    }
    ...
    {
        $context = $this->getCurrentUserContext();
        return $this->approvalHierarchy->evaluateApprovalScope($scope, $request, $context);
    }
     ...
     * เรียกใช้จาก IbApprovalHierarchy
     */
    private function getUserVisibleStages(array $context)
    {
        return $this->approvalHierarchy->getUserVisibleStages($context);
    }
    ...
    private function getPreviousStage(array $request, string $currentStage): ?string
    {
        $versionPlanCode = $this->resolveVersionPlanCode($request);
        $flow = $this->getApprovalFlow($versionPlanCode);
        $index = array_search($currentStage, $flow, true);
    ...
    private function updateRequestStatus($id)
    {
        ...
        if (in_array($statusInput, ['approved', 'approve'], true)) {
            ...
            if ($nextStatus) {
                $result = $this->ibRequestsModel->updateStatus($id, $nextStatus, $updated_by, ['clear_return' => true]);
            } else {
                $result = $this->ibRequestsModel->updateStatus($id, 'approved', $updated_by, ['clear_return' => true]);
            }
        } elseif (in_array($statusInput, ['rejected', 'reject'], true)) {
            $result = $this->ibRequestsModel->updateStatus($id, 'rejected', $updated_by, ['clear_return' => true]);
            ...
            $result = $this->ibRequestsModel->updateStatus($id, $targetStatus, $updated_by, ['clear_return' => true]);
        } else {
    ...
    private function processApprovalAction($id)
    {
        ...
        $workflowModeRaw = $request['workflow_mode'] ?? 1;
        $workflowMode = (int) $workflowModeRaw;
        if ($workflowMode <= 0) {
            $workflowMode = 1;
        }
        $statusOptions = [];
        ...
        if (in_array($action, ['approve', 'approved'], true)) {
            $nextStatus = $this->getNextStageStatus($request);
            $newStatus = $nextStatus ?: 'approved';
            $statusOptions['clear_return'] = true;
        } else {
            if ($workflowMode === 2) {
                $returnStage = $this->getPreviousStage($request, $currentStage);
                $newStatus = $returnStage ? $this->makeStageStatus($returnStage) : 'draft';
                $statusOptions['return'] = [
                    'is_returned' => true,
                    'returned_from_stage' => $currentStage,
                    'returned_reason' => $comments,
                    'returned_by_user_id' => $updated_by,
                ];
            } else {
                $newStatus = 'rejected';
                $statusOptions['clear_return'] = true;
        ...
        $updated = $this->ibRequestsModel->updateStatus($id, $newStatus, $updated_by, $statusOptions);
    ...
    private function saveSapExport($payload)
    {
        ...
        // สร้าง folder ถ้ายังไม่มี (ใช้ helper function)
        try {
            $this->ensureDirectoryExists($baseDir);
        } catch (Exception $e) {
            // error_log("❌ SAP Directory Error: {$e->getMessage()}");
            $this->responseError("Failed to create SAP directory: {$baseDir}", 500);
            return;
        }

        // ตรวจสอบว่าสามารถเขียนได้หรือไม่
        if (!is_writable($baseDir)) {
            // error_log("❌ ไม่สามารถเขียนไฟล์ในโฟลเดอร์ SAP: {$baseDir}");
            $this->responseError("SAP directory is not writable: {$baseDir}", 500);
            return;
        }
    ...
    private function saveFormData()
    {
        ...
        try {
            ...
            if ($result !== false) {
                ...
                if ($this->isStageStatus($status)) {
                    $this->ibRequestsModel->updateStatus(
                        $result['id'],
                        $status,
                        $requestData['created_by'] ?? ($requestData['updated_by'] ?? '1'),
                        ['clear_return' => true]
                    );
                }
    ...
    private function updateFormData($id)
    {
        ...
        try {
            ...
            // รักษา attachment_files ถ้ามีการส่งมา (สำหรับการลบไฟล์)
            if (isset($formData['attachment_files'])) {
                $requestData['attachment_files'] = $formData['attachment_files'];
            }
            ...
            // ถ้ามีการอัพเดต attachment_files ให้ลบไฟล์เก่าที่ไม่ใช้แล้ว
            if (isset($formData['attachment_files'])) {
                $oldPaths = $this->ibRequestsModel->getAttachmentFilePaths($id);
                $newPaths = $formData['attachment_files'];
                $this->deleteRemovedAttachments($oldPaths, $newPaths);
            }
            ...
            if ($result) {
                ...
                if ($this->isStageStatus($status)) {
                    $this->ibRequestsModel->updateStatus($id, $status, $requestData['updated_by'], ['clear_return' => true]);
                    $updatedRequest = $this->ibRequestsModel->readRequest($id);
                    if ($updatedRequest) {
                        $updatedRequest['attachments'] = $attachments;
                    }
    ...
    private function getUploadedPdfFiles()
    {
        ...
        $targets = [];
        // Support both 'pdf_attachments[]' (from FormData with multiple files) and 'pdf_attachments' (legacy)
        if (isset($_FILES['pdf_attachments'])) {
            //   error_log("📎 DEBUG: pdf_attachments structure: " . print_r($_FILES['pdf_attachments'], true));
            $targets[] = $_FILES['pdf_attachments'];
        }
        if (isset($_FILES['pdf_attachments_1'])) {
            //   error_log("📎 DEBUG: pdf_attachments_1 structure: " . print_r($_FILES['pdf_attachments_1'], true));
            $targets[] = $_FILES['pdf_attachments_1'];
        }

        foreach ($targets as $target) {
            $normalized = $this->normalizeFilesArray($target);
            //   error_log("📎 DEBUG: Normalized files count: " . count($normalized));
        ...
        //   error_log("📎 DEBUG: Total files to process: " . count($files));
        return $files;
    }
    ...
    /**
     * ตรวจสอบว่าไฟล์ที่อัปโหลดเป็นไฟล์ประเภทที่อนุญาตหรือไม่
    ...
    private function validatePdfUploads(array $files)
    {
        ...
        foreach ($files as $file) {
            ...
            // อนุญาตไฟล์หลายประเภท
            $allowedExtensions = ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'jpg', 'jpeg', 'png', 'gif', 'zip', 'rar'];
            ...
            $isValidMime = $mime ? in_array($mime, $allowedMimes, true) : true;

            if (!in_array($extension, $allowedExtensions) || !$isValidMime) {
                if ($finfo) {
                    finfo_close($finfo);
                }
                return 'อนุญาตเฉพาะไฟล์: PDF, Word (.doc, .docx), Excel (.xls, .xlsx), PowerPoint (.ppt, .pptx), รูปภาพ (.jpg, .png, .gif), ZIP/RAR';
            }
        }
    ...
    {
        //   error_log("📎 handleAttachments START: requestId={$requestId}, files=" . count($files));

        $existingPaths = $this->ibRequestsModel->getAttachmentFilePaths($requestId);
        $paths = is_array($existingPaths) ? $existingPaths : [];
        //   error_log("📎 Existing paths: " . json_encode($existingPaths));

        if (empty($files)) {
            //   error_log("📎 No files to process, updating with existing paths");
        ...
        foreach ($files as $index => $file) {
            //   error_log("📎 Processing file #{$index}: " . ($file['name'] ?? 'unknown'));

            $errorCode = $file['error'] ?? UPLOAD_ERR_NO_FILE;

            if ($errorCode !== UPLOAD_ERR_OK) {
                //   error_log("❌ File #{$index} has error code: {$errorCode}");
                continue;
            }
            ...
            $storedFileName = implode('_', $storedFileNameParts) . '.' . $extension;
        ...
        //   error_log("📎 Total paths to save: " . count($paths) . " (processed: {$filesProcessed})");
        //   error_log("📎 Paths: " . json_encode($paths));

        $updateResult = $this->ibRequestsModel->updateAttachmentFilesColumn($requestId, $paths);
    ...
    /**
     * ลบไฟล์ที่ถูกเอาออกจาก attachment list
     */
    private function deleteRemovedAttachments($oldPaths, $newPaths)
    {
        ...
        foreach ($removedPaths as $path) {
            ...
            // แปลง web path เป็น file system path
            $filePath = $projectRoot . str_replace('/interfaces2/', '/', $path);

            if (file_exists($filePath) && is_file($filePath)) {
                if (unlink($filePath)) {
                    //   error_log("🗑️ ลบไฟล์สำเร็จ: {$filePath}");
                } else {
                    //   error_log("❌ ลบไฟล์ไม่สำเร็จ: {$filePath}");
                }
            } else {
                //   error_log("⚠️ ไฟล์ไม่พบ: {$filePath}");
            }
        }
    }

    /**
     * สร้างโฟลเดอร์พร้อมตรวจสอบ permission (รองรับทั้ง Linux และ Windows)
     *
     * @param string $path - path ของโฟลเดอร์ที่ต้องการสร้าง
     * @param bool $throwException - โยน exception เมื่อสร้างไม่สำเร็จ (default: true)
     * @return bool - true ถ้าโฟลเดอร์มีอยู่หรือสร้างสำเร็จ
     * @throws Exception - ถ้าสร้างไม่สำเร็จและ $throwException = true
     */
    private function ensureDirectoryExists($path, $throwException = true)
    {
        ...
        if (!$created && !is_dir($path)) {
            ...
            // บน Windows: แจ้งวิธีแก้ไข
            if (DIRECTORY_SEPARATOR === '\\') {
                $fixMessage = "\n\nวิธีแก้ไข (Windows Server):\n";
                $fixMessage .= "1. สร้างโฟลเดอร์ด้วยตนเอง: {$path}\n";
                $fixMessage .= "2. คลิกขวาโฟลเดอร์ → Properties → Security\n";
                $fixMessage .= "3. เพิ่ม IIS_IUSRS และ IUSR ให้มีสิทธิ์ Full Control\n";
                $fixMessage .= "หรือรัน: icacls \"{$path}\" /grant \"IIS_IUSRS:(OI)(CI)F\" /T";
                // error_log($fixMessage);

                $message .= "\n\nกรุณาตรวจสอบสิทธิ์โฟลเดอร์ (IIS_IUSRS, IUSR)";
            }
    ...
    private function transformFormData($formData)
    {
        ...
        $requestData = [
            ...
            'work_period_number' => !empty($formData['work_period_number'])
                ? $formData['work_period_number']
                : (!empty($formData['installmentCount']) ? $formData['installmentCount'] : null),
