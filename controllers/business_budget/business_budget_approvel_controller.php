...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($post['position_code'] != $user['position_code']) {
        arrx('s');
    }
    ...
    if(isset($post['approve_status']) && $post['approve_status'] == 'approve') {
        ...
        if($post['position_code'] == 'APBR' || $post['position_code'] == getVersionApprove($post['version_name'])) {
            $post['approve_status'] = 'rejected';
        } else {
            $post['approve_position_code'] = getNextReject($post['position_code']);
            $post['approve_status'] = 'waitting';
        }   
