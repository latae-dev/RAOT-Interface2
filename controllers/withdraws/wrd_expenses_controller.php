...
class ExpensesController
{
        ...
        switch ($method) {
            case 'GET':
                if ($_GET['action'] == 'expenses_search') {
                    $this->readSearch($_GET['key'], $_GET['limit'], $_GET['offset'], $_GET['expenses_number'], $_GET['start_date'], $_GET['end_date'], $_GET['status'], $_GET['req_user_code'], '', '', '');
                    ...
                    $this->readSearch($_GET['key'], $_GET['limit'], $_GET['offset'], $_GET['expenses_number'], $_GET['start_date'], $_GET['end_date'], $_GET['status'], $_GET['req_user_code'], $_GET['req_position_code'], $_GET['req_depart_code'], $_GET['search_user_code']);
    ...
    {
        $edit_main_state = $this->expensesModel->updateWrdPlanApprove($data, $id, $position_code);
        $return_id = $this->WrdExpensesApproveHistoryModel->createApproveLogs($data, $id, $position_code);
    ...
    public function approveExpensesMulti($data, $id, $position_code)
    {
        ...
        foreach ($records as $record) {
            ...
            $updated = $this->expensesModel->updateWrdPlanApproveMulti($data, $targetId, $position_code);
            if (!$updated) {
                $updateFailed[] = $targetId;
                continue;
            }

            $logId = $this->WrdExpensesApproveHistoryModel->createApproveLogsMulti($data, $targetId, $position_code);
            if (!$logId) {
                $logFailed[] = $targetId;
                continue;
            }
    ...
    public function readSearch($key, $limit, $offset, $expenses_number, $start_date, $end_date, $status, $req_user_code, $req_position_code, $req_depart_code, $search_user_code)
    {
        $respond = $this->expensesModel->readAllSearch($key, $limit, $offset, $expenses_number, $start_date, $end_date, $status, $req_user_code, $req_position_code, $req_depart_code, $search_user_code);
