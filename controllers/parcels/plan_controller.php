...
class PlanController
{
    ...
    public function processRequest()
    {
        ...
        switch ($method) {
            case 'GET':
                ...
                } elseif ($_GET['action'] == 'plan_search_list') {
                ...
                } elseif ($_GET['action'] == 'plan_search_merge') {
                    $this->readAllSearchMerge($_GET['doc_type'], $_GET['limit'], $_GET['offset'], $_GET['plan_number'], $_GET['start_date'], $_GET['end_date'], $_GET['status_plan']);
                } elseif ($_GET['action'] == 'plan_search_approve') {
                    $this->readAllSearchApprovelist($_GET['doc_type'], $_GET['limit'], $_GET['offset'], $_GET['plan_number'], $_GET['start_date'], $_GET['end_date'], $_GET['status_plan']);
    ...
    public function readAllSearchlist($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan)
    {
        $userData = $_SESSION['user_data'];

        $respond = $this->planModel->readAllSearchlist($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan, $userData);
        echo  json_encode($respond);
    }

    public function readAllSearchApprovelist($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan)
    {
        $userData = $_SESSION['user_data'];

        $respond = $this->planModel->readAllSearchApprovelist($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan, $userData);
        echo  json_encode($respond);
    }

    public function readAllSearchMerge($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan)
    {
        $userData = $_SESSION['user_data'];

        // $this->debug($userData,true);

        $respond = $this->planModel->readAllSearchMerge($doc_type, $limit, $offset, $plan_number, $start_date, $end_date, $status_plan, $userData);
        echo  json_encode($respond);
    }
    ...
    protected function debug($model, $exit = null) {
