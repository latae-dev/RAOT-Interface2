...
// SSR: ดึงข้อมูลจากฐานข้อมูลโดยตรง
$users = [];
try {
    $configDir = __DIR__ . '/../configs';
    $modelDir  = __DIR__ . '/../models';

    require_once $configDir . '/database-wrapper.php';
    require_once $configDir . '/database.php';
    require_once $modelDir . '/user/user_model.php';

    $database = new Database();
    $db = $database->connect('raot_db_qas');
    if ($db) {
        $userModel = new UserModel($db);
        $result = $userModel->readUserAll();
        if (is_array($result)) {
            $users = $result;
        }
    }
} catch (Exception $e) {
    // Log error แต่ไม่หยุดการแสดงผลหน้า
    error_log('user-list.php SSR error: ' . $e->getMessage());
}
...
function formatDatePhp($dateString)
{
    ...
    try {
        ...
        return date('d/m/', $timestamp) . $thaiYear . ' ' . date('H:i', $timestamp);
    } catch (Exception $e) {
        return $dateString;
    }
}
            ...
            <div class="card custom-card">
                                ...
                                <tbody id="users-tbody">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($users as $index => $user): ?>
                                            <tr
                                                data-user-code="<?php echo esc($user['user_code'] ?? ''); ?>"
                                                data-user-fname="<?php echo esc($user['user_fname'] ?? ''); ?>"
                                                data-user-lname="<?php echo esc($user['user_lname'] ?? ''); ?>"
                                                data-user-status="<?php echo esc($user['user_status'] ?? ''); ?>">
                                                <td class="text-center"><?php echo $index + 1; ?></td>
                                                <td><strong><?php echo esc($user['user_code'] ?? ''); ?></strong></td>
                                                <td><?php echo esc(trim(($user['user_fname'] ?? '') . ' ' . ($user['user_lname'] ?? ''))) ?: '-'; ?></td>
                                                <td><?php echo esc($user['user_email'] ?? '-'); ?></td>
                                                <td><?php echo esc($user['position_name'] ?? '-'); ?></td>
                                                <td><?php echo esc($user['depart_name'] ?? '-'); ?></td>
                                                <td class="text-center"><?php echo getStatusBadgePhp($user['user_status'] ?? ''); ?></td>
                                                <td class="text-center"><?php echo formatDatePhp($user['created_at'] ?? ''); ?></td>
                                                <td class="text-center">
                                                    <div class="hstack gap-2 fs-15 justify-content-center">
                                                        <button type="button" class="btn btn-icon btn-sm btn-success-transparent rounded-pill" onclick="viewUser(<?php echo (int)$user['id']; ?>)" title="ดูรายละเอียด">
                                                            <i class="ri-eye-line"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-icon btn-sm btn-info-transparent rounded-pill" onclick="editUser(<?php echo (int)$user['id']; ?>)" title="แก้ไข">
                                                            <i class="ri-edit-line"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-icon btn-sm btn-danger-transparent rounded-pill" onclick="deleteUser(<?php echo (int)$user['id']; ?>)" title="ลบ">
                ...
                <div class="card-footer border-top-0">
                    <div class="d-flex align-items-center flex-wrap gap-2">
                        <div>
                            <label class="me-1 small">แสดง</label>
                            <select class="form-select form-select-sm d-inline-block" style="width:auto;" id="per-page-select">
                                <option value="10" selected>10</option>
                                <option value="25">25</option>
                                <option value="50">50</option>
                                <option value="100">100</option>
                            </select>
                            <label class="ms-1 small">รายการ</label>
                        </div>
                        <div id="pagination-info" class="ms-2">กำลังแสดง <?php echo min(10, count($users)); ?> จาก <?php echo count($users); ?> รายการ</div>
                        <div class="ms-auto">
