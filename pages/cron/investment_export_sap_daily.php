...
try {
    ...
    // Fetch approved requests with SAP status = 'not_synced'
    // Note: Don't include 'current_user_id' or 'approval_scope' to bypass user filtering
    // Export all not_synced records regardless of date

    logMessage("Fetching all approved requests with sap_status = 'not_synced'");
    ...
    // No header - only data rows (Tab-separated values)
    // Add data rows
    foreach ($data as $item) {
        ...
        $budgetType = '8';
        $amountRaw = $item['total_amount'] ?? '0';
        $amount = is_numeric($amountRaw)
            ? number_format((float)$amountRaw, 2, '.', '')
            : '0.00';
        // Remove tabs from item_name to prevent breaking the TSV format
    ...
    // Create file content
    $txtContent = implode($NEWLINE, $lines);
