...
$expenses_allowance            = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_allowance'], 2)) : '';
$expenses_allowance_days       = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_allowance_days'], 1)) : '';
$allowance_total               = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_allowance_total'], 2)) : arabicToThaiNumber(number_format((float)$total_allowance, 2));
$expenses_accommodation        = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_accommodation'], 2)) : '';
$expenses_accommodation_days   = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_accommodation_days'], 1)) : '';
$accommodation_total           = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_accommodation_total'], 2)) : arabicToThaiNumber(number_format((float)$total_stay, 2));
$expenses_transportation       = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_transportation'], 2)) : 'รายละเอียดตามเอกสารแนบ';
$transport_total               = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_transportation_total'], 2)) : arabicToThaiNumber(number_format((float)$total_transport, 2));
$expenses_moving               = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_moving'], 1)) : '-';
$expenses_moving_km            = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_moving_km'], 2)) : '-';
$expenses_moving_total         = arabicToThaiNumber(number_format((float)$main['expenses_moving_total'], 2));
$expenses_vehicle_number       = arabicToThaiNumber($main['expenses_vehicle_number']);
$expenses_other                = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_other'], 2)) : 'รายละเอียดตามเอกสารแนบ';
$other_total                   = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_other_total'], 2)) : arabicToThaiNumber(number_format((float)$total_other_expenses, 2));
$expenses_food_per_meal        = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_food_per_meal'], 2)) : '-';
$expenses_food_meals           = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_food_meals'], 1)) : '-';
$food_total                    = ($expenses_group == 1) ? arabicToThaiNumber(number_format((float)$main['expenses_food_total'], 2)) : arabicToThaiNumber(number_format((float)$total_meal, 2));
$total_expenses                = arabicToThaiNumber(number_format((float)$main['expenses_total'], 2));
$plan_total                    = arabicToThaiNumber($main['plan_total']);
$expenses_amount               = arabicToThaiNumber(number_format((float)$main['expenses_amount'], 2));
...
$child_total = arabicToThaiNumber(number_format((float)$child_total_val, 2));
...
{
  return '<span style="display:inline-block;width:12px;height:12px;border:1.2px solid #333;border-radius:50%;text-align:center;line-height:12px;font-size:16px;font-family:DejaVu Sans, Sarabun;margin-right:3px;">'
    . ($checked ? '✔' : '&nbsp;') .
    '</span> ' . $label;
}
...
<p class="right">
วันที่ ' . dottedLine(arabicToThaiNumber($start_date), 50, 'center', 'dotted') . ' เดือน ' . dottedLine(thaiMonth($start_month), 90, 'center', 'dotted') . ' พ.ศ. ' . dottedLine(arabicToThaiNumber($start_year), 50, 'center', 'dotted') . '
</p>
...
<p>
ข้าพเจ้า ' . dottedLine($full_name, 270, 'center', 'dotted') . ' ตำแหน่ง ' . dottedLine($position_name, 170, 'center', 'dotted') . ' ระดับ ' . dottedLine($level_name, 130, 'center', 'dotted') . '
</p>
<p>
สังกัด ' . dottedLine($affiliation, 175, 'center', 'dotted') . ' พร้อมด้วยคณะ ' . dottedLine($group_count, 50, 'center', 'dotted') . ' คน &nbsp; เดินทางไปปฏิบัติงานตามที่ได้รับอนุมัติโดยออกเดินทางจาก
</p>
...
' . radioMark($expenses_group === "1", "ข้าพเจ้า") . '
' . radioMark($expenses_group === "2", "คณะเดินทาง") . '
ดังนี้
</p>
...
<table width="100%" border="0">
  ...
  <tr>
    <td>ค่าพาหนะ ' . dottedLine($expenses_transportation, 424, 'center', 'dotted') . ' </td>
    <td>รวม ' . dottedLine($transport_total, 140, 'center', 'dotted') . ' บาท</td>
  </tr>
  <tr>
    <td>ค่าขนย้ายสิ่งของส่วนตัว ระยะทาง ' . dottedLine($expenses_moving_km, 243, 'center', 'dotted') . ' กิโลเมตร</td>
  ...
  <tr>
    <td>ค่าใช้จ่ายอื่น ๆ ' . dottedLine($expenses_other, 405, 'center', 'dotted') . ' </td>
...
$html2 .= '
  <tr>
    <th class= "th-vertical-center" width="10%" rowspan="2" style="height:20px;font-size:14pt">วัน เดือน ปี</th>
    ...
    <th class= "th-vertical-center" width="20%" colspan="2" style="font-size:14pt">ค่าพาหนะส่วนตัว</th>
    <th class= "th-vertical-center" width="40%" rowspan="2" style="font-size:14pt">หมายเหตุ</th>
  </tr>
  <tr>
    ...
    <th style="font-size:14pt">ที่พักแรม</th>
    <th style="font-size:14pt">ระยะทาง<br>(กม.)</th>
    <th style="font-size:14pt">เงินชดเชย<br>(บาท)</th>
  </tr>';
...
ประกอบใบเบิกค่าใช้จ่ายในการเดินทาง' . dottedLine("", 180, 'center', 'dotted') . ' สังกัด ' . dottedLine("", 180, 'center', 'dotted') . ' ลงวันที่ ' . dottedLine("", 70, 'center', 'dotted') . ' เดือน ' . dottedLine("(3)" . "", 150, 'center', 'dotted') . ' พ.ศ. ' . dottedLine("", 70, 'center', 'dotted') . '
</p>

<table border="1">
  <tr class="center bold">
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="5%">ลำดับที่</th>
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="15%">ชื่อ (4)</th>
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="10%">ตำแหน่ง/ระดับ (5)</th>
    <th class="center-middle" style="font-size:14pt" colspan="5" width="25%">ค่าใช้จ่าย (บาท) (6)</th>
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="10%">รวม (บาท) (7)</th>
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="10%">ลายมือชื่อผู้รับเงิน (8)</th>
    <th class="center-middle" style="font-size:14pt;min-width:240px" rowspan="2" width="25%">หมายเหตุ</th>
  </tr>
  <tr class="center bold">
    <th style="font-size:14pt">ค่าเบี้ยเลี้ยง</th>
    <th style="font-size:14pt">ค่าที่พัก</th>
    <th style="font-size:14pt">ค่าพาหนะ</th>
    <th style="font-size:14pt">ค่าใช้จ่าย<br>อื่นๆ</th>
    <th style="font-size:14pt">หักค่าอาหาร<br>ฝึกอบรม</th>
  </tr>';
...
foreach ($traveling_group_data as $r) {
  ...
  $html3 .= '
    <tr>
      ...
      <td style="font-size:14pt;">' . $r['fullname'] . '</td>
...
$html3 .= '
  <tr class="bold">
    <td colspan="3" class="center" style="font-size:14pt">รวมเงิน (9)</td>
    ...
    <td class="center" colspan="2" style="font-size:14pt"> ตามสัญญาเงินยืมฯเลขที่ ' . dottedLine("(11)" . "", 70, 'center', 'dotted') . ' วันที่ ' . dottedLine("", 90, 'center', 'dotted') . ' </td>
  </tr>
</table>
...
<table style="width: 100%; border: none; font-size: 12pt;">
  <tr>
    <td style="width: 72%; border: none;">
        ...
        <p style="font-size:14pt;"> 1. ค่าเบี้ยเลี้ยงและค่าที่พักให้ระบุอัตราวันละ และจำนวนวันที่ขอเบิกของแต่ละบุคคลในช่องหมายเหตุ </p>
        <p style="font-size:14pt;"> 2. ให้ผู้มีสิทธิเบิกลงนามเป็นผู้ลงลายมือชื่อผู้รับเงิน </p>
        <p style="font-size:14pt;"> 3. ผู้จ่ายเงินหมายถึงผู้ซึ่งอยู่ในตำแหน่งผู้จัดทำใบค่าใช้จ่ายในการเดินทางไปปฏิบัติงาน
...
header('Content-Disposition: inline; filename="ใบเบิกค่าใช้จ่ายในการเดินทางไปปฏิบัติงาน.pdf"');
$final->SetTitle("ใบเบิกค่าใช้จ่ายในการเดินทางไปปฏิบัติงาน", true);
$final->Output('I');
exit;
