...
<style>
    ...
    /* Improve readability for important text */
    ...
    /* ป้องกันเส้นซ้ำเมื่อ detail-row ซ้อนกัน */
    ...
    /* ปรับ padding และ margin ให้เท่ากันสำหรับข้อมูลทั่วไป */
    .card-body>.row.g-3 {
        padding-left: 0;
        padding-right: 0;
        margin-left: 0;
...
<div class="container-fluid">
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">รายละเอียดคำขอตั้งงบลงทุน</h1>
        <div class="ms-md-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item fw-semibold"><a href="javascript:void(0);">ระบบคำขอตั้งงบลงทุน</a></li>
                    <li class="breadcrumb-item fw-semibold"><a href="investment-budget-list.php">รายการคำขอตั้งงบลงทุน</a></li>
                    <li class="breadcrumb-item fw-semibold active" aria-current="page">รายละเอียด</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="col-xl-12">
            <div class="card custom-card">
                <div class="card-body text-center py-5">
                    ...
                    <a href="investment-budget-list.php" class="btn btn-primary mt-3">
                        <i class="ri-arrow-left-line me-2"></i>กลับไปรายการ
                    </a>
                </div>
            </div>
        </div>
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between flex-wrap gap-2">
                            <a href="investment-budget-list.php" class="btn btn-light">
...
<script>
    ...
    // ฟังก์ชันสำหรับจัดการ Back URL
    window.getBackUrl = function() {
        // 1. ลองดึงจาก query parameter ก่อน (มีความแม่นยำสูงสุด)
        ...
        // 2. ใช้ document.referrer เป็น fallback
        if (document.referrer) {
            const referrer = new URL(document.referrer);
            const referrerPath = referrer.pathname;

            // ตรวจสอบว่า referrer เป็นหน้าที่รู้จักหรือไม่
            const knownPages = [
                'investment-budget-list.php',
                'investment-budget-approvel-list.php',
                'investment-budget-report-list.php'
            ];

            if (knownPages.some(page => referrerPath.includes(page))) {
                return referrer.pathname + referrer.search;
            }
        }

        // 3. Default fallback
        return 'investment-budget-list.php';
    };

    // เรียกใช้ทันทีเพื่อ update ปุ่มกลับทั้งหมด
    document.addEventListener('DOMContentLoaded', function() {
        const backUrl = window.getBackUrl();
        console.log('🔙 Back URL:', backUrl);

        // อัพเดตทุกปุ่มกลับในหน้า
        document.querySelectorAll('a[href="investment-budget-list.php"]').forEach(function(link) {
            // เช็คว่าเป็นปุ่มกลับจริงๆ (มี icon arrow-left)
            if (link.querySelector('.ri-arrow-left-line')) {
                link.href = backUrl;
                console.log('✅ Updated back button href to:', backUrl);
            }
        });
    });
</script>
...
<!-- Investment Budget View Detail JS -->
