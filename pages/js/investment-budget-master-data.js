...
class MasterDataManager {
  ...
  async initialize() {
    ...
    try {
      ...
      await Promise.all([
        ...
        this.loadAssetNumbers() // โหลดหมายเลขครุภัณฑ์
      ]);
  ...
  */
  ...
  /**
   * โหลดข้อมูลหมายเลขครุภัณฑ์ (12 หลัก)
   */
  async loadAssetNumbers() {
    try {
      const response = await fetch(
        '../controllers/investment_budget/ib_requests_controller.php?action=asset_numbers'
      );
      ...
      if (result.status === 'success' && Array.isArray(result.data)) {
        this.data.assetNumbers = result.data;
        console.log(`🔢 โหลดหมายเลขครุภัณฑ์: ${this.data.assetNumbers.length} รายการ`);
      } else {
        console.warn('⚠️ ไม่พบข้อมูลหมายเลขครุภัณฑ์');
        this.data.assetNumbers = [];
      }
    } catch (error) {
      console.error('❌ ไม่สามารถโหลดข้อมูลหมายเลขครุภัณฑ์ได้:', error);
      this.data.assetNumbers = [];
    }
  }
