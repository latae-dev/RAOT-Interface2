...
class InvestmentBudgetReportManager {
  ...
  parseBooleanFlag(value) {
    ...
    const truthy = ['1', 'y', 'yes', 'true', 'active', 'enable', 'enabled', 'on'];
    return truthy.includes(normalized);
  }

  isBudgetApproved(status) {
    return String(status || '').toLowerCase() === 'approved';
  }

  isBudgetRejected(status) {
    return String(status || '').toLowerCase() === 'rejected';
  }

  formatBudgetApprovalBadge(status) {
    const normalized = String(status || '').toLowerCase();
    switch (normalized) {
      case 'approved':
        return '<span class="badge bg-success">อนุมัติงบแล้ว</span>';
      case 'rejected':
        return '<span class="badge bg-danger">ไม่อนุมัติงบ</span>';
      default:
        return '<span class="badge bg-secondary">รอพิจารณางบ</span>';
    }
  }
  ...
  renderCurrentTab() {
    ...
    // Tabs 2-4 ต้องแสดงเฉพาะที่อนุมัติงบประมาณแล้ว
    if (tab !== 'all') {
      data = data.filter((item) =>
        this.isBudgetApproved(item?.budget_approval_status)
      );
    }
  ...
  renderAllTab(data, startIndex) {
    ...
    const rows = data
      .map((item, i) => {
        ...
        const budgetBadge = this.formatBudgetApprovalBadge(
          item.budget_approval_status
        );
        const budgetStatus = String(item.budget_approval_status || '')
          .trim()
          .toLowerCase();
        const budgetButton =
          this.canBudgetApprove(item) && budgetStatus === 'pending'
            ? `<a href="investment-budget-view-detail.php?id=${item.id}&budgetApproval=1" class="btn btn-sm btn-warning" title="พิจารณาอนุมัติงบประมาณ">พิจารณาอนุมัติงบประมาณ</a>`
            : '';

        const approveBtn =
          item.id &&
          String(item.budget_approval_status || '')
            .trim()
            .toLowerCase() === 'pending'
            ? `<a href="investment-budget-approve.php?id=${item.id}" class="btn btn-icon btn-sm btn-info-transparent rounded-pill" title="พิจารณาคำขอ"><i class="fe fe-check-circle"></i></a>`
            : '';
      ...
      .map((item, i) => {
        ...
        return `
            <tr class="text-center">
                ...
                <td>${requestNumber}</td>
                <td>${itemName}</td>
                <td>${item.quantity == 0 || item.quantity == '0' ? '-' : (item.quantity ?? '-')}</td>
                ...
                ${countUnitCell}
                <td>
                    ${this.getActionButtonsHtml(item, 'equipment')}
                </td>
            </tr>
        `;
      })
      ...
      .map((item, i) => {
        ...
        return `
            <tr class="text-center">
                ...
                <td>
                    ${this.getActionButtonsHtml(item, 'construction')}
                </td>
            </tr>
        `;
      })
      ...
      .map((item, i) => {
        ...
        return `
            <tr class="text-center">
                ...
                <td>
                    ${this.getActionButtonsHtml(item, 'plantation')}
                </td>
            </tr>
        `;
      })
  ...
  getActionButtonsHtml(item, tab = 'all') {
    ...
    if (!this.canEditItem(item)) {
      return `${containerStart}${approveButton}${viewButton}${containerEnd}`;
    }
    ...
    return `${containerStart}${saveButton}${approveButton}${viewButton}${containerEnd}`;
  }

  canBudgetApprove(item) {
    ...
    const isPendingBudget = !item?.budget_approval_status;

    return isPendingBudget && canHeadOffice && canInvestment;
  }
  ...
  formatUnitPriceForExport(unitPrice, totalAmount, quantity) {
    ...
    if (hasQty) {
      ...
      result = Number.isFinite(total) ? total / qtyNum : 0;
    } else {
      const price = Number(
  ...
  showError(message) {
    ...
    ['all', 'equipment', 'construction', 'plantation'].forEach((tab) => {
      ...
      if (tbody) {
        const colspan = tab === 'all' ? 9 : tab === 'equipment' ? 14 : 13;
  ...
  async exportToExcel(tab = null) {
    ...
    try {
      ...
      // ใช้เฉพาะคำขอที่ budget_approval_status = pending
      const pendingBudget = data.filter((item) => {
        const status = String(item.budget_approval_status ?? '').trim().toLowerCase();
      ...
      rows.push([
        ...
        item.item_name || '-',
        item.fiscal_year_id || '-',
        this.formatCurrencyExport(item.quantity || 0),
        this.formatCurrencyExport(
          this.formatUnitPriceForExport(
            item.unit_price,
            this.getApprovedAmount(item),
            item.quantity
          )
        ),
        this.formatCurrencyExport(this.getApprovedAmount(item)),
            this.normalizeDisplayValue(item.reason) || '-',
            this.normalizeDisplayValue(item.equipment_code_type_code) || '-',
            this.normalizeDisplayValue(item.equipment_type_code) || '-',
            this.normalizeDisplayValue(item.asset_category) || '-',
            this.normalizeDisplayValue(item.count_unit_name) || '-'
          ]);
        });
      ...
      const filename =
        currentTab === 'all'
          ? `export_report_consider_${timestamp}.xlsx`
          : `รายงานคำขอตั้งงบลงทุน_${currentTab}_${timestamp}.xlsx`;
  ...
  async exportSAPReport() {
    ...
    try {
      ...
      allData.forEach((item) => {
        ...
        const amount = this.formatAmountForExport(
          this.getApprovedAmount(item)
        );
  ...
  async exportProjectReport() {
    ...
    try {
      ...
      allData.forEach((item) => {
        ...
        // กรอกงบประมาณ - ใช้งบที่อนุมัติแล้ว
        const budgetAmount = this.formatAmountForExport(
          this.getApprovedAmount(item)
        );
