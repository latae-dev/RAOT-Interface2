  ...
  class InvestmentBudgetApprovalListManager {
    ...
    loadCurrentUser() {
      try {
        ...
        console.warn(
          'ไม่สามารถอ่านข้อมูลผู้ใช้งานจาก sessionStorage ได้',
          error
        );
        return null;
      }
    ...
    parseBooleanFlag(value) {
      ...
      return ['1', 'true', 't', 'yes', 'y', 'active', 'ใช้', 'ใช้งาน'].includes(
        normalized
      );
    }

    hasBudgetApprovalPrivileges() {
      if (!this.currentUser) {
        return false;
      }
      ...
      return canHeadOffice && canInvestment;
    }
    ...
    getBudgetApprovalBadge(request) {
      ...
      // หรือถ้า API ไม่ส่ง budget_approval_status มาให้ (undefined/null)
      if (
    ...
    getActionButtons(request) {
      // สร้าง returnUrl จากหน้าปัจจุบัน
      const returnUrl = encodeURIComponent(
        window.location.pathname + window.location.search
      );
      const viewUrl = `investment-budget-view-detail.php?id=${request.id}&returnUrl=${returnUrl}`;
      ...
      const hasBudgetPrivileges = this.hasBudgetApprovalPrivileges();
      const isPendingBudgetApproval =
        hasBudgetPrivileges &&
        request.status &&
        request.status.toLowerCase() === 'approved' &&
        (!budgetApprovalStatus || budgetApprovalStatus === 'pending');

      let approveButton = '';
      if (isPendingBudgetApproval) {
        approveButton = `
            <a href="${approveUrl}" class="btn btn-icon btn-sm btn-warning-transparent rounded-pill" title="ดูรายละเอียด (อยู่นอกสิทธิ์ของคุณ)">
                <i class="bx bx-list-check"></i>
            </a>
        `;
      } else {
        const actionButtonClass = request.can_approve
          ? 'btn-warning-transparent'
          : 'btn-info-transparent';
        const actionButtonTitle = request.can_approve
          ? `ตรวจ${levelLabel ? ` - สิทธิระดับ ${levelLabel}` : ''}`
          : 'ดูรายละเอียด (อยู่นอกสิทธิ์ของคุณ)';
        approveButton = `
            <a href="${approveUrl}"
               class="btn btn-icon btn-sm ${actionButtonClass} rounded-pill"
