class InvestmentBudgetRequestView {
  ...
  renderBudgetApprovalSection() {
    ...
    // ถ้าคำขอยังไม่ได้รับการอนุมัติ และ user ไม่ใช่ HQ ที่มีสิทธิ์พิจารณา ให้ซ่อน section
    if (rawStatus !== 'approved') {
      // ถ้าเป็นโหมดพิจารณาและเป็น HQ user หรือมีสิทธิ์อนุมัติคำขอ ให้แสดง section ได้
      if (!isApprovalMode || (!isHQUser && !canApproveRequest)) {
  ...
  async handleApprovalSubmit() {
    ...
    try {
      ...
      const isApprovePage = currentPath.includes('investment-budget-approve.php');
