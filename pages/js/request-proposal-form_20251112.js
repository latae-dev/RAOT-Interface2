    ...
    try {
        ...
        if (result && result.status === 'success' && Array.isArray(result.data)) {
            ...
            const expenPrOptions = [
                ...
                ...result.data.map(item => ({
                    value: item.id,
                    text: `${item.expen_code} - ${item.expen_name}`,
                    name: item.expen_name,
                    code: item.expen_code
                }))
            ];
 ...
 * ใช้ข้อมูลจาก liability_controller เหมือนกับรายการภาระผูกพันธ์
 */
...
/**
 * ดึงรายการ WRD Fund จาก API และเติมลง dropdown
 */
async function loadWrdFunds() {
    try {
        // หา dropdown WRD Fund ทั้งหมด (ไม่เลือก, A สินทรัพย์, K ศูนย์ต้นทุน)
        ...
        if (result && result.status === 'success' && Array.isArray(result.data)) {
            ...
            // เติมข้อมูลใน dropdown WRD Fund ทั้งหมด (ไม่เลือก, A สินทรัพย์, K ศูนย์ต้นทุน)
...
function fillDropdownsInRowForAsset(row) {
    ...
    if (assetCategorySelect && window.assetCategoryData) {
        ...
        // เพิ่ม options ใหม่ (ใช้ข้อมูลจาก asset_category)
