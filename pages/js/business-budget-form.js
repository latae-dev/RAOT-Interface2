...
function calculateRowActivity($row) {
    ...
    if (qty > 0 && price > 0) {
        ...
        $row.find(".amount").val(formattedAmount);
        $row.find(".total_amount").val(formattedAmount);
    } else {
        $row.find(".amount").val("");
...
function parseNumber(val) {
    let valueToParse = (val === null || val === undefined) ? "" : val.toString();
...
function calculateSummary() {
    ...
    $("#activity_table .total_summary_amount").val(formatNumber(total));
}
...
function calculateRowExpense($row) {
    ...
    $row.find(".allowance_amount").val(formatNumber(allowance_amount));
    $row.find(".transport_amount").val(formatNumber(transport_amount));
    $row.find(".stay_amount").val(formatNumber(stay_amount));
    $row.find(".fuel_amount").val(formatNumber(fuel_amount));
    $row.find(".total_activity").val(formatNumber(total_activity));
}
...
function calculateSummaryExpense() {
    ...
    $("#expense_table .total_summary_allowance").val(formatNumber(total_allowance));
    $("#expense_table .total_summary_transport").val(formatNumber(total_transport));
    $("#expense_table .total_summary_stay").val(formatNumber(total_stay));
    $("#expense_table .total_summary_fuel").val(formatNumber(total_fuel));
    $("#expense_table .total_summary_amount").val(formatNumber(total_amount));
...
function calculateRowOperation($row) {
    ...
    $row.find(".operation_plan_target").val(formatNumber(total));
}
...
function calculateSummaryBudget() {
    ...
    $("#budget_summary_amount").val(formatNumber(total_summary_budget));
}
...
function updateAllGroupsSummary() {
    $("#operation_plan tbody.group").each(function() {
        ...
        $summary.find(".total_summary_weight_amount").val(formatNumber(sum));
    });
}
...
$(document).ready(function () {
    ...
    $("#activity_id").on("change", function () {
        ...
        table3.find("tbody.summary").not(":first").remove();
