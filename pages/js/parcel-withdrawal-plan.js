...
async function createData(doc_type) {
    ...
    // ระดับคำขอ: ใช้ประเภทหน่วยสังกัด (depart_type_name) ก่อน — ตรงกับตารางหน่วย; จากนั้น branch_type เซสชัน แล้วจึงรหัสตำแหน่ง
    ...
    if(doc_type === 'master'){
        ...
        if(date_deli.value == ''){
            $(date_deli).addClass('is-invalid');
            checked = false;
        }else{
            $(date_deli).removeClass('is-invalid');
        }

        let date_requ = document.querySelector('input[name="date_requ"]');
        if(date_requ.value == ''){
            $(date_requ).addClass('is-invalid');
            checked = false;
        }else{
            $(date_requ).removeClass('is-invalid');
        }

        let date_approv = document.querySelector('input[name="date_approv"]');
        if(date_approv.value == ''){
            $(date_approv).addClass('is-invalid');
            checked = false;
        }else{
            $(date_approv).removeClass('is-invalid');
        }

        let time_deli_plan = document.querySelector('input[name="time_deli_plan"]');
        if(time_deli_plan.value == ''){
            $(time_deli_plan).addClass('is-invalid');
            checked = false;
        }else{
            $(time_deli_plan).removeClass('is-invalid');
        }

        let time_pick_plan = document.querySelector('input[name="time_pick_plan"]');
        if(time_pick_plan.value == ''){
            $(time_pick_plan).addClass('is-invalid');
            checked = false;
        }else{
            $(time_pick_plan).removeClass('is-invalid');
        }
    }
