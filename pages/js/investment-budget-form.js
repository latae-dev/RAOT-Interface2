...
class InvestmentBudgetForm {
  ...
  setupFormLogic() {
    ...
    // ควบคุมการแสดง dropdown จำนวนงวด
    this.setupInstallmentCountVisibility();

    // เตรียม Choices.js สำหรับจำนวนงวด
    this.initializeInstallmentCountChoices();
  ...
  showCardByAssetTypeId(assetTypeId) {
    ...
    if (targetCard) {
      ...
      // Initialize asset number dropdowns ถ้าเป็น Card 1 (ทดแทน)
      if (normalizedId === '1') {
        console.log('🔢 กำลัง initialize asset number dropdowns สำหรับ Card 1...');
        // ใช้ setTimeout เล็กน้อยเพื่อให้ DOM render เสร็จและ master data พร้อม
        setTimeout(() => {
          this.initializeAssetNumberDropdowns();
        }, 500);
      }
    } else {
  ...
  /**
   * แสดง dropdown จำนวนงวดเมื่อเลือก "เป็นงวดงาน" (id = 2)
   */
  setupInstallmentCountVisibility() {
    const installmentSelects = document.querySelectorAll(
      'select[id^="installment-work-select-"]'
    );

    if (!installmentSelects.length) {
      return;
    }

    const toggleVisibility = (selectElement) => {
      if (!selectElement) return;
      const assetTypeId = (selectElement.id || '').replace(
        'installment-work-select-',
        ''
      );
      const wrapper = document.getElementById(
        `installment-count-wrapper-${assetTypeId}`
      );
      const countSelect = document.getElementById(
        `installment-count-select-${assetTypeId}`
      );

      if (!wrapper || !countSelect) {
        return;
      }

      const isInstallment = String(selectElement.value) === '2';
      ...
      if (isInstallment) {
        ...
        countSelect.required = false;
        countSelect.value = '';
        this.clearElementValidationState(countSelect);
        this.updateFeedbackVisibility(countSelect);
      }
    };

    installmentSelects.forEach((select) => {
      select.addEventListener('change', () => toggleVisibility(select));
      toggleVisibility(select); // ประมวลผลค่าเริ่มต้น
    });
  }
  ...
  validateForApproval() {
    ...
    const requiresInstallment = ['3', '7', '8', '9'].includes(
      formData.assetType
    ); // ใช้งวดงาน
    const requiresMultiYearPlan = !['8'].includes(formData.assetType); // Asset Type 8 ไม่ต้องเลือกแผนเบิกจ่าย > 1 ปี
    ...
    // ===== 18. การเบิกจ่ายหลายปี =====
    {
      ...
      if (el && shouldValidateField(el)) {
        if (requiresMultiYearPlan) {
          ...
          // Asset Type 8: ฟิลด์นี้ไม่ใช่เงื่อนไขจำเป็น
          markValidEl(el);
        }
      }
    }
    ...
    if (requiresInstallment) {
      ...
      // จำนวนงวด (แสดงเมื่อเลือก "เป็นงวดงาน" id = 2)
      const countEl = resolveEl('installment-count-select', {
        tag: 'select'
      });
      const needsCount =
        String(formData.workPeriod) === '2' ||
        String(formData.work_period_number) === '2';
      const countValue =
        formData.installmentCount || formData.work_period_number || '';
      if (needsCount) {
        if (isEmpty(countValue)) {
          addErr('กรุณาเลือกจำนวนงวด', countEl);
        } else {
          markValidEl(countEl);
        }
      } else if (countEl) {
        this.clearElementValidationState(countEl);
        this.updateFeedbackVisibility(countEl);
      }
    }
  ...
  /**
   * สร้าง Choices.js ให้ dropdown จำนวนงวด (1-24) สำหรับการ์ดที่ต้องใช้งวดงาน
   */
  initializeInstallmentCountChoices() {
    const selects = document.querySelectorAll(
      'select.installment-count-select'
    );
  ...
  populateSerialNumbers(container, serialNumbers = []) {
    ...
    if (!targetContainer) {
      ...
      if (fallbackTbody) {
        const cardNumber = this.getCardNumberFromTbodyId(fallbackTbody.id);
        fallbackTbody.innerHTML = '';
        const values = serialNumbers.length ? serialNumbers : [''];
        values.forEach((value, index) => {
          const row = this.createNewSerialRow(value, cardNumber, index);
          fallbackTbody.appendChild(row);
        });

        // Initialize dropdowns สำหรับทุก card
        this.initializeAssetNumberDropdowns();
      }
      return;
    }
    ...
    const cardNumber = this.getCardNumberFromTbodyId(tbody.id);
    const values = serialNumbers.length ? serialNumbers : [''];
    tbody.innerHTML = '';
    ...
    // ตรวจสอบว่ามีอย่างน้อย 1 แถว (ไม่ต้องเช็คเพราะสร้างไว้แล้วด้านบน)
    // if (!tbody.querySelector('.equipment-input') && !tbody.querySelector('.equipment-select')) {
    //   tbody.appendChild(this.createNewSerialRow('', cardNumber, 0));
    // }
  ...
  initializeEquipmentSerialManager() {
    ...
    // เริ่มต้น dropdown สำหรับ Card 1 (ถ้ามี)
    this.initializeAssetNumberDropdowns();
  ...
  addNewSerialRow(tbodyId) {
    ...
    // หา card number จาก tbody id (เช่น machinery-serial-tbody-1 -> card 1)
    const cardNumber = this.getCardNumberFromTbodyId(tbodyId);
    console.log('🔍 Adding row to card:', cardNumber, 'from tbody:', tbodyId);

    // นับจำนวนแถวที่มีอยู่เพื่อกำหนด row index
    const existingRows = tbody.querySelectorAll('.equipment-row');
    const rowIndex = existingRows.length;

    const newRow = this.createNewSerialRow('', cardNumber, rowIndex);
    tbody.appendChild(newRow);

    // ทุก card ใช้ dropdown - populate options และ initialize Choices.js
    const newSelect = newRow.querySelector('.equipment-select');
    if (newSelect) {
      this.populateAssetNumberOptions(newSelect);
      this.initializeSingleAssetNumberDropdown(newSelect);
    }
   ...
   * @param {string} value - ค่าที่จะใส่ (สำหรับ input) หรือ ID ที่จะเลือก (สำหรับ dropdown)
   * @param {number} cardNumber - หมายเลขการ์ด (1-10) เพื่อระบุว่าใช้ dropdown หรือ input
   * @param {number} rowIndex - index ของแถว (สำหรับ dropdown id)
   */
  createNewSerialRow(value = '', cardNumber = null, rowIndex = 0) {
    ...
    // ทุก card ใช้ dropdown หมด
    row.innerHTML = `
      <td>
        <select class="form-control equipment-select"
                id="asset-number-select-${cardNumber}-${rowIndex}"
  ...
  removeSerialRow(row) {
    ...
    // ถ้าเหลือแถวเดียว ไม่ให้ลบ - แค่ clear ค่า
  ...
  getSerialNumbers(tbodyId) {
    ...
    // รวบรวมค่าจาก input แบบเดิม (Card 2-10)
    ...
    // รวบรวมค่าจาก dropdown (Card 1)
    const selects = tbody.querySelectorAll('.equipment-select');
    selects.forEach((select) => {
    ...
    const fallbackSelects = document.querySelectorAll(
      '.replacement-asset-container .equipment-select'
    );
  ...
  /**
   * ดึงหมายเลข card จาก tbody ID
   * @param {string} tbodyId - ID ของ tbody (เช่น machinery-serial-tbody-1)
   * @returns {number|null} หมายเลข card หรือ null ถ้าไม่พบ
   */
  getCardNumberFromTbodyId(tbodyId) {
    if (!tbodyId) return null;

    // Extract number from ID like "machinery-serial-tbody-1"
    const match = tbodyId.match(/machinery-serial-tbody-(\d+)/);
    return match ? parseInt(match[1], 10) : null;
  }

  /**
   * Populate options ให้กับ asset number dropdown
   * @param {HTMLSelectElement} selectElement - Element ที่จะเพิ่ม options
   */
  populateAssetNumberOptions(selectElement) {
    if (!selectElement) return;

    // ล้าง options เดิม (ยกเว้น placeholder)
    selectElement.innerHTML = '<option value="">เลือกหมายเลขครุภัณฑ์</option>';

    // ดึงข้อมูลจาก master data
    const assetNumbers = this.masterData?.data?.assetNumbers || [];

    if (assetNumbers.length === 0) {
      console.warn('⚠️ ไม่มีข้อมูลหมายเลขครุภัณฑ์');
      return;
    }
    ...
    assetNumbers.forEach((asset) => {
      const option = document.createElement('option');
      option.value = asset.id; // เก็บ ID
      option.textContent = `${asset.code} - ${asset.name}`; // แสดง code + name
      option.dataset.code = asset.code;
      option.dataset.name = asset.name;
      selectElement.appendChild(option);
    });

    console.log(`✅ เพิ่ม ${assetNumbers.length} รายการใน dropdown`);
  }

  /**
   * Initialize Choices.js สำหรับ dropdown เดียว
   * @param {HTMLSelectElement} selectElement - Element ที่จะ initialize
   */
  initializeSingleAssetNumberDropdown(selectElement) {
    if (!selectElement || typeof Choices === 'undefined') {
      console.warn('⚠️ ไม่สามารถ initialize Choices.js ได้');
      return;
    }

    // ทำลาย instance เก่าถ้ามี
    this.destroyChoicesInstance(selectElement);
    ...
    const instance = new Choices(selectElement, {
      ...
      shouldSort: false,
      placeholder: true,
      placeholderValue: 'เลือกหมายเลขครุภัณฑ์',
    ...
    console.log('✅ Initialize Choices.js สำหรับ', selectElement.id);
  }

  /**
   * Initialize asset number dropdowns สำหรับ Card 1
   */
  initializeAssetNumberDropdowns() {
    console.log('🔢 เริ่มต้น asset number dropdowns...');

    // ตรวจสอบว่า master data พร้อมใช้งาน
    if (!this.masterData || !this.masterData.data || !this.masterData.data.assetNumbers) {
      console.warn('⚠️ Master data ยังไม่พร้อม ไม่สามารถ initialize dropdowns ได้');
      return;
    }

    const assetNumbers = this.masterData.data.assetNumbers;
    if (assetNumbers.length === 0) {
      console.warn('⚠️ ไม่มีข้อมูลหมายเลขครุภัณฑ์ใน master data');
      return;
    }

    console.log(`📊 พบข้อมูลหมายเลขครุภัณฑ์: ${assetNumbers.length} รายการ`);

    // หา dropdowns ทั้งหมดใน Cards 1, 2, 4, 7, 10
    const allDropdowns = document.querySelectorAll('.equipment-select[data-card]');

    if (allDropdowns.length === 0) {
      console.log('⚠️ ไม่พบ dropdown (อาจจะยังไม่ได้เลือกประเภทข้อมูลทดแทน)');
      return;
    }

    console.log(`📋 พบ ${allDropdowns.length} dropdown(s) ทั้งหมด`);

    allDropdowns.forEach((select, index) => {
      const cardNumber = select.dataset.card;
      console.log(`  - กำลัง populate dropdown ${index + 1} (Card ${cardNumber})...`);
      this.populateAssetNumberOptions(select);
      this.initializeSingleAssetNumberDropdown(select);
    });

    console.log(`✅ Initialize ${allDropdowns.length} dropdowns สำเร็จ`);
  }
  ...
  initializePriceCalculator() {
    ...
    // ตั้งค่า comma formatting แบบ real-time สำหรับ price inputs ทั้งหมด
  ...
  /**
   * ตั้งค่า comma formatting แบบ real-time สำหรับ price inputs
   */
  setupPriceInputFormatting() {
    console.log('💵 ตั้งค่า comma formatting สำหรับ price inputs...');

    // ใช้ InvestmentBudgetCommon.setupPriceFormatting() สำหรับ price inputs ทั้งหมด
    if (window.InvestmentBudgetCommon && window.InvestmentBudgetCommon.setupPriceFormatting) {
      // Setup formatting สำหรับ price inputs ทั้งหมด (price-1 ถึง price-10)
      window.InvestmentBudgetCommon.setupPriceFormatting('[id^="price-"]', 2);
      console.log('✅ ตั้งค่า comma formatting สำหรับ price inputs เรียบร้อย');
    } else {
      console.warn('⚠️ InvestmentBudgetCommon ไม่พร้อมใช้งาน - ข้ามการตั้งค่า comma formatting');
    }
  }
  ...
  calculateTotal(config) {
    ...
    // ใช้ parseFormattedNumber เพื่อ parse ค่าจาก price input ที่มี comma
    let price = 0;
    if (window.InvestmentBudgetCommon && window.InvestmentBudgetCommon.parseFormattedNumber) {
      price = window.InvestmentBudgetCommon.parseFormattedNumber(priceInput.value);
    } else {
      // Fallback: ลบ comma ด้วยตัวเอง
  ...
  getFormData() {
    ...
    const formData = {
      ...
      Price: (() => {
        const priceInput = document.getElementById(`price-${assetType}`);
        if (!priceInput || !priceInput.value) return '';
        // Parse comma-formatted price value
        if (window.InvestmentBudgetCommon && window.InvestmentBudgetCommon.parseFormattedNumber) {
          return window.InvestmentBudgetCommon.parseFormattedNumber(priceInput.value);
        }
      ...
      operationYear:
        ...
        '',
      workPeriod:
        (['3', '8', '7', '9'].includes(assetType) &&
          this.masterData.getValue(`installment-work-select-${assetType}`)) ||
        '',
      installmentCount:
        (['3', '7', '8', '9'].includes(assetType) &&
          document.getElementById(`installment-count-select-${assetType}`)
            ?.value) ||
        '',
      work_period_number:
        (['3', '7', '8', '9'].includes(assetType) &&
  ...
  async loadFormData(data) {
    ...
    try {
      ...
      // โหลด version_plan_id ถ้ามี ถ้าไม่มีให้ใช้ค่า default ตามระดับ user
      if (data.version_plan_id) {
        ...
        // ถ้าไม่มี version_plan_id ให้ดึงค่า default จาก user hierarchy
        if (
          window.userHierarchyData &&
          window.userHierarchyData.allowed_version_plan_id
        ) {
          await this.masterData.setValue(
            'version-plan-select',
            window.userHierarchyData.allowed_version_plan_id
          ...
          console.log(
            '✅ Set default version plan from user hierarchy:',
            window.userHierarchyData.allowed_version_plan_id
          );
        }
      }
      ...
      if (data.unit_price) {
        ...
        if (priceInput) {
          ...
          const formattedPrice = window.InvestmentBudgetCommon && window.InvestmentBudgetCommon.formatCurrency
            ? window.InvestmentBudgetCommon.formatCurrency(data.unit_price)
            : this.formatNumber(data.unit_price);
      ...
      if (data.work_period_number && data.asset_type_id) {
        const countSelect = document.getElementById(
          `installment-count-select-${data.asset_type_id}`
        );
        if (countSelect) {
          countSelect.value = data.work_period_number;
  ...
  loadExistingAttachments(attachmentFiles) {
    console.log('========================================');
    console.log('📎 loadExistingAttachments CALLED!');
    console.log('📎 attachmentFiles:', attachmentFiles);
    console.log('📎 typeof:', typeof attachmentFiles);
    console.log('========================================');
    ...
    containers.forEach((container) => {
      ...
      files.forEach((file, index) => {
        ...
        // If fileName is empty, try to extract from path
        if (!fileName && typeof filePath === 'string') {
          fileName = filePath.split('/').pop() || 'ไฟล์แนบ ' + (index + 1);
        } else if (!fileName) {
          fileName = 'ไฟล์แนบ ' + (index + 1);
        }

        const fileIconInfo = this.getFileIconAndType(fileName);
        ...
        const html = `
          <div class="d-flex align-items-center">
            <i class="${fileIconInfo.icon} ${fileIconInfo.colorClass} me-2 fs-5"></i>
            <div class="d-flex flex-column">
              <a href="${filePath}" target="_blank" class="text-decoration-none">
                ${fileName}
              </a>
              ${fileIconInfo.typeName ? `<small class="text-muted">${fileIconInfo.typeName}</small>` : ''}
            </div>
          </div>
        ...
        console.log('📎 Created file item HTML for index', index, ':', html);
        container.appendChild(fileItem);
      });
    });

    // เพิ่ม event listener สำหรับปุ่มลบ (ใช้ event delegation)
    containers.forEach((container) => {
      // Remove old listener first to prevent duplicate
      container.removeEventListener('click', this._handleRemoveFile);
      ...
      this._handleRemoveFile = (e) => {
        ...
        if (btn) {
          ...
          console.log('🗑️ Removing file at index:', fileIndex, 'from files:', files);
          this.removeExistingFile(fileIndex, files);
        }
      };

      container.addEventListener('click', this._handleRemoveFile);
    });
  }
  ...
  removeExistingFile(fileIndex, files) {
    console.log('🗑️ removeExistingFile called with index:', fileIndex, 'files:', files);
    ...
    Swal.fire({
      ...
      console.log('🗑️ Swal result:', result);
      if (result.isConfirmed) {
        try {
          // แสดง loading
          window.showLoading('กำลังลบไฟล์...');

          // ลบไฟล์จาก array
          files.splice(fileIndex, 1);

          // อัปเดต database ด้วยรายการไฟล์ที่เหลือ
          ...
          const userContextPayload = this.getUserContextPayload();
          if (userContextPayload) {
            formData.append('user_context', userContextPayload.user_context);
            if (userContextPayload.user_context_encoding) {
              formData.append('user_context_encoding', userContextPayload.user_context_encoding);
            }
          }

          const response = await fetch(
            `../controllers/investment_budget/ib_requests_controller.php?action=update&id=${this.requestId}`,
            {
              method: 'POST',
              body: formData
            }
          ...
          Swal.fire('เกิดข้อผิดพลาด!', error.message || 'ไม่สามารถลบไฟล์ได้', 'error');
        }
      }
    });
  }
  ...
  /**
   * Get file icon and type information based on file extension
   * Returns icon class, color class, and type name for display
   */
  getFileIconAndType(fileName) {
    if (!fileName) {
      return { icon: 'bi bi-file-earmark-fill', colorClass: 'text-muted', typeName: '' };
    }

    const ext = fileName.toLowerCase().split('.').pop();
    console.log('📎 Getting icon for file:', fileName, 'extension:', ext);

    const iconMap = {
      // PDF
      pdf: { icon: 'bi bi-file-earmark-pdf-fill', colorClass: 'text-danger', typeName: 'PDF' },

      // Word
      doc: { icon: 'bi bi-file-earmark-word-fill', colorClass: 'text-primary', typeName: 'Word' },
      docx: { icon: 'bi bi-file-earmark-word-fill', colorClass: 'text-primary', typeName: 'Word' },

      // Excel
      xls: { icon: 'bi bi-file-earmark-excel-fill', colorClass: 'text-success', typeName: 'Excel' },
      xlsx: { icon: 'bi bi-file-earmark-excel-fill', colorClass: 'text-success', typeName: 'Excel' },

      // PowerPoint
      ppt: { icon: 'bi bi-file-earmark-ppt-fill', colorClass: 'text-warning', typeName: 'PowerPoint' },
      pptx: { icon: 'bi bi-file-earmark-ppt-fill', colorClass: 'text-warning', typeName: 'PowerPoint' },

      // Images
      jpg: { icon: 'bi bi-file-earmark-image-fill', colorClass: 'text-info', typeName: 'รูปภาพ' },
      jpeg: { icon: 'bi bi-file-earmark-image-fill', colorClass: 'text-info', typeName: 'รูปภาพ' },
      png: { icon: 'bi bi-file-earmark-image-fill', colorClass: 'text-info', typeName: 'รูปภาพ' },
      gif: { icon: 'bi bi-file-earmark-image-fill', colorClass: 'text-info', typeName: 'รูปภาพ' },

      // Archives
      zip: { icon: 'bi bi-file-earmark-zip-fill', colorClass: 'text-secondary', typeName: 'ZIP' },
      rar: { icon: 'bi bi-file-earmark-zip-fill', colorClass: 'text-secondary', typeName: 'RAR' }
    };

    const result = iconMap[ext] || { icon: 'bi bi-file-earmark-fill', colorClass: 'text-muted', typeName: '' };
