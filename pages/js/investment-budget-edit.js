...
class InvestmentBudgetEditManager {
  ...
  populateForm() {
    ...
    if (assetTypeId) {
      ...
      if (
        ...
        // จำนวนงวด (work_period_number) หากเลือกเป็น "เป็นงวดงาน"
        if (this.requestData.work_period_number) {
          this.setDropdownValue(
            `installment-count-select-${assetTypeId}`,
            this.requestData.work_period_number
          );
        }
      }
    }
  ...
  showRelevantCard() {
    if (!this.requestData || !this.requestData.asset_type_name) {
      console.warn('⚠️ รอข้อมูล requestData หรือ asset_type_name...');
      return;
    }
   ...
   */
  populateItems() {
    console.log('🔍 DEBUG populateItems - START:', {
      hasRequestData: !!this.requestData,
      hasItemName: !!this.requestData?.item_name,
      requestData: this.requestData
    });

    if (!this.requestData) {
      console.warn('⚠️ populateItems RETURN - No requestData');
      return;
    }
    ...
    // โหลด serial numbers (หมายเลขครุภัณฑ์ทดแทน) - ย้ายออกมานอก if block
    console.log('🔍 Check replacement_asset_numbers:', {
      exists: !!item.replacement_asset_numbers,
      value: item.replacement_asset_numbers,
      type: typeof item.replacement_asset_numbers,
      isArray: Array.isArray(item.replacement_asset_numbers)
    });

    if (item.replacement_asset_numbers) {
      // หา tbody ของการ์ดที่แสดงอยู่
      const allCards = document.querySelectorAll('.card.custom-card');
      console.log('🔍 Total cards:', allCards.length);

      const visibleCard = Array.from(allCards).find(
        (card) =>
          card.style.display !== 'none' &&
          card.querySelector('[id^="machinery-serial-tbody-"]')
      );

      console.log('🔍 Visible card:', visibleCard ? 'Found' : 'Not found');

      if (visibleCard) {
        const tbody = visibleCard.querySelector(
          '[id^="machinery-serial-tbody-"]'
        );
        console.log('🔍 tbody:', tbody ? tbody.id : 'Not found');

        if (tbody) {
          console.log('🔍 Debug - item.replacement_asset_numbers:', item.replacement_asset_numbers);
          console.log('🔍 Debug - typeof:', typeof item.replacement_asset_numbers);
          console.log('🔍 Debug - Array.isArray:', Array.isArray(item.replacement_asset_numbers));
      ...
      console.warn('⚠️ No replacement_asset_numbers in item data');
    }
  }
  ...
  populateSerialNumbers(tbodyId, serialNumbers) {
    ...
    if (serialNumbersArray.length === 0) {
      this.addEmptySerialRow(tbody, cardNumber);
      return;
    }
    ...
    // ทุก card ใช้ dropdown - populate และ initialize
    console.log(`🔢 กำลัง populate dropdowns สำหรับ Card ${cardNumber}...`);
    setTimeout(() => {
      this.initializeEditModeDropdowns(tbody, serialNumbersArray);
    }, 300);
  }
   ...
   */
  addEmptySerialRow(tbody, cardNumber = null) {
    const rowIndex = tbody.querySelectorAll('.equipment-row').length;
    const row = this.createSerialRow('', cardNumber, rowIndex);
   ...
   */
  addSerialRow(tbody, serialNumber, isLast, cardNumber = null, rowIndex = 0) {
    const row = this.createSerialRow(serialNumber, cardNumber, rowIndex);
    tbody.appendChild(row);
   ...
   * @param {string} value - ค่า serial number (สำหรับ input) หรือ ID (สำหรับ dropdown)
   * @param {number} cardNumber - หมายเลข card (1-10)
   * @param {number} rowIndex - index ของแถว
   */
  createSerialRow(value, cardNumber = null, rowIndex = 0) {
    const row = document.createElement('tr');
    row.className = 'equipment-row';

    // ถ้าไม่ระบุ cardNumber ให้พยายามหาจาก context
    if (!cardNumber && this.requestData) {
      cardNumber = this.requestData.asset_type_id || null;
    }
    ...
    row.innerHTML = `
          <td>
              <select class="form-control equipment-select"
                      id="asset-number-select-edit-${cardNumber}-${rowIndex}"
  ...
  /**
   * Initialize dropdowns สำหรับ edit mode
   */
  initializeEditModeDropdowns(tbody, selectedIds) {
    console.log('🔢 Initialize Edit Mode Dropdowns', selectedIds);

    if (!window.investmentBudgetForm || !window.investmentBudgetForm.masterData) {
      console.warn('⚠️ Master data ยังไม่พร้อม');
      return;
    }

    const assetNumbers = window.investmentBudgetForm.masterData.data.assetNumbers || [];
    if (assetNumbers.length === 0) {
      console.warn('⚠️ ไม่มีข้อมูล asset numbers');
      return;
    }

    const dropdowns = tbody.querySelectorAll('.equipment-select');
    console.log(`📋 พบ ${dropdowns.length} dropdowns`);

    dropdowns.forEach((select, index) => {
      // Populate options
      select.innerHTML = '<option value="">เลือกหมายเลขครุภัณฑ์</option>';
      ...
      // Set selected value
      if (selectedIds && selectedIds[index]) {
        select.value = selectedIds[index];
        console.log(`  - Dropdown ${index}: selected ID = ${selectedIds[index]}`);
      }
      ...
      if (typeof Choices !== 'undefined') {
        ...
        console.log(`  ✅ Initialize Choices.js สำหรับ ${select.id}`);
      }
    });

    console.log('✅ Initialize Edit Mode Dropdowns เสร็จสิ้น');
  }
  ...
  renderAttachments(attachments) {
    ...
    if (hasAttachments) {
      const listItems = attachments
        .map((attachment, index) => {
          ...
          const fileIcon = this.getFileIconAndType(displayName);

          return `
                    <li class="list-group-item d-flex justify-content-between align-items-start">
                        <div class="me-3 flex-grow-1">
                            <a href="${url}" target="_blank" rel="noopener" class="fw-semibold">
                                <i class="${fileIcon.icon} me-2 ${fileIcon.colorClass}"></i>${displayName}
                            </a>
                            ${
                              uploadedAt
                            ...
                            ${fileIcon.typeName ? `<div class="small text-muted">${fileIcon.typeName}${sizeText ? ' • ' + sizeText : ''}</div>` : sizeText ? `<div class="small text-muted">${sizeText}</div>` : ''}
                        </div>
                        <button type="button" class="btn btn-sm btn-danger-light remove-attachment-btn" data-index="${index}" title="ลบรายการ">
                            <i class="bi bi-dash"></i> ลบรายการ
                        </button>
                    </li>
                `;
    ...
    containers.forEach((container) => {
      ...
      const removeButtons = container.querySelectorAll('.remove-attachment-btn');
      removeButtons.forEach((btn) => {
        btn.addEventListener('click', async (e) => {
          const index = parseInt(e.currentTarget.getAttribute('data-index'), 10);
          await this.removeAttachment(index, attachments);
        });
      });
    });
  }

  async removeAttachment(index, attachments) {
    try {
      ...
      const userContext = this.buildEncodedUserContext();
      if (userContext) {
        formData.append('user_context', userContext.value);
        if (userContext.encoding) {
          formData.append('user_context_encoding', userContext.encoding);
        }
      }

      const response = await fetch(
        `../controllers/investment_budget/ib_requests_controller.php?action=update_form&id=${this.requestId}`,
        {
          method: 'POST',
          body: formData
        }
  ...
  getFileIconAndType(fileName) {
    ...
    return iconMap[ext] || { icon: 'bi bi-file-earmark-fill', colorClass: 'text-muted', typeName: '' };
  }
