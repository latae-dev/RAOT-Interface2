...
class InvestmentBudgetReportManager {
  ...
  async updateFiltersFromData(items) {
    ...
    if (Array.isArray(items)) {
      items.forEach((item) => {
        ...
        if (item.budget_source_id && item.budget_source_name) {
  ...
  async loadMasterData() {
    try {
      const headers = this.getUserContextHeaders();
      const [codeTypes, equipTypes, parameters] = await Promise.all([
  ...
  categorizeData(items) {
    ...
    this.data.all = items;
    this.data.equipment = items.filter((item) => {
      const type = getType(item);
      return ['1', '2', '4', '7', '10'].includes(type);
    });
    this.data.construction = items.filter((item) => {
      const type = getType(item);
      return ['3', '8', '9'].includes(type);
    });
    this.data.plantation = items.filter((item) => {
      const type = getType(item);
  ...
  renderAllTab(data, startIndex) {
    ...
    const rows = data
      .map((item, i) => {
        ...
        const branchName = this.escapeHtml(item.branch_name || '-');
  ...
  renderEquipmentTab(data, startIndex) {
    ...
    const rows = data
      .map((item, i) => {
        ...
        const isSapSuccess = item.sap_status === 'success';
        const modalCellClass = isSapSuccess ? '' : 'modal-cell';
        const modalToggle = isSapSuccess ? '' : 'data-bs-toggle="modal"';
        const cursorStyle = isSapSuccess ? 'style="cursor: not-allowed; opacity: 0.6;"' : 'style="cursor: pointer;"';
        ...
        return `
            <tr class="text-center">
                ...
                <td>${reason}</td>
                <td class="${modalCellClass}" ${modalToggle} data-bs-target="#modal-equipment-code-type" data-field="equipment_code_type" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          equipmentCodeTypeValue
        )}" data-sap-status="${item.sap_status || ''}" ${cursorStyle}>${this.escapeHtml(equipmentCodeTypeLabel)}</td>
                <td>${this.escapeHtml(item.asset_type_code || '-')}</td>
                <td class="${modalCellClass}" ${modalToggle} data-bs-target="#modal-equipment-type" data-field="equipment_type" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          equipmentTypeValue
        )}" data-sap-status="${item.sap_status || ''}" ${cursorStyle}>${this.escapeHtml(equipmentTypeLabel)}</td>
                <td class="${modalCellClass}" ${modalToggle} data-bs-target="#modal-count-units" data-field="count_units" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          countUnitValue
        )}" data-sap-status="${item.sap_status || ''}" ${cursorStyle}>${this.escapeHtml(countUnitLabel)}</td>
                <td>
  ...
  renderConstructionTab(data, startIndex) {
    ...
    if (!data.length) {
      tbody.innerHTML =
        '<tr><td colspan="13" class="text-center">ไม่พบข้อมูล</td></tr>';
      return;
    }
      ...
      .map((item, i) => {
        ...
        return `
            <tr class="text-center">
                ...
                <td>${this.formatCurrency(item.total_amount)}</td>
                <td>${reason}</td>
                <td class="modal-cell" data-bs-toggle="modal" data-bs-target="#modal-equipment-code-type" data-field="equipment_code_type" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          equipmentCodeTypeValue
        )}">${this.escapeHtml(equipmentCodeTypeLabel)}</td>
                <td>${this.escapeHtml(item.asset_type_code || '-')}</td>
                <td class="modal-cell" data-bs-toggle="modal" data-bs-target="#modal-equipment-type" data-field="equipment_type" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          equipmentTypeValue
        )}">${this.escapeHtml(equipmentTypeLabel)}</td>
                <td class="modal-cell" data-bs-toggle="modal" data-bs-target="#modal-count-units" data-field="count_units" data-id="${
                  item.id
                }" data-value="${this.escapeAttr(
          countUnitValue
        )}">${this.escapeHtml(countUnitLabel)}</td>
                <td>${this.escapeHtml(
  ...
  getActionButtonsHtml(item) {
    ...
    if (this.savedItems.has(idKey) || sapStatus === 'success') {
      return `${containerStart}${viewButton}${containerEnd}`;
    }
  ...
  showError(message) {
    ...
    ['all', 'equipment', 'construction', 'plantation'].forEach((tab) => {
      ...
      if (tbody) {
        const colspan = tab === 'all' ? 8 : tab === 'equipment' ? 14 : 13;
  ...
  showLoadingState() {
    const configs = [
      { tab: 'all', columns: 8 },
      { tab: 'equipment', columns: 14 },
      { tab: 'construction', columns: 13 },
      { tab: 'plantation', columns: 13 }
    ];
  ...
  async exportToExcel(tab = null) {
    ...
    try {
      ...
      let excludedCount = 0;
      if (currentTab === 'equipment' || currentTab === 'construction' || currentTab === 'plantation') {
        const originalLength = data.length;
        data = data.filter((item) => {
          const hasEquipmentCodeType = item.equipment_code_type_id && item.equipment_code_type_id !== '';
          const hasEquipmentType = item.equipment_type_id && item.equipment_type_id !== '';
  ...
  async exportAssetReport() {
    ...
    try {
      ...
      allData.forEach((item) => {
        const branchName = item.branch_name || 'ไม่ระบุหน่วยงาน';
        ...
        .forEach((branchName) => {
          ...
          items.forEach((item) => {
            rows.push([
              ...
              item.branch_name || item.department_name || branchName,
              item.item_name || '-',
              item.quantity || '-',
              this.formatCurrency(item.unit_price),
              this.formatCurrency(item.total_amount),
  ...
  async exportSAPReport() {
    // ส่งออกข้อมูล SAP Interface เป็น .txt ไฟล์ คั่นด้วย |
    try {
      let allData = this.data.all || [];
      ...
      const excludedCount = (allData.filter(item => item.sap_status !== 'success').length) - filteredData.length;
      if (excludedCount > 0) {
        console.log(`⚠️ ไม่สามารถ export ได้ ${excludedCount} รายการ เนื่องจากข้อมูลครุภัณฑ์ไม่ครบ (ต้องมี: รหัสครุภัณฑ์, ชนิด ประเภทครุภัณฑ์, หน่วยนับ)`);
      }
      ...
      // เพิ่มหัวตาราง (ตามรูปแบบ SAP) คั่นด้วย |
      //   lines.push(
      //     [
      //       'ศูนย์เงินทุน',
      //       'เงินทุน',
      ...
      allData.forEach((item) => {
        ...
        // รายการ (แทนที่ | ด้วย - เพื่อไม่ให้ขัดกับ delimiter)
      ...
      if (result.status === 'success') {
        console.log('✅ Export SAP Report (.txt) สำเร็จ');
        this.showToast(
          'บันทึกไฟล์ SAP สำเร็จที่: ' + result.filepath,
          'success'
        );
        ...
        console.error('❌ Export SAP Report ล้มเหลว:', result.message);
        this.showToast(
          'ไม่สามารถบันทึกไฟล์ SAP ได้: ' + result.message,
          'error'
        );
      }
    } catch (error) {
  ...
  async exportProjectReport() {
    ...
    try {
      ...
      allData.forEach((item) => {
        ...
        // กรอกงบประมาณ - total_amount
        const budgetAmount = this.formatCurrency(item.total_amount);
