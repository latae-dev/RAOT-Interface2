...
$(document).ready(function() {
    ...
    function getActionButtons(proposal) {
        ...
        // ปุ่มดู (แสดงเสมอ)
        buttons += `<a href="request-proposal-view.php?id=${proposal.id}" class="btn btn-icon btn-sm btn-success-transparent rounded-pill" title="ดู"><i class="fe fe-eye"></i></a>`;
        
        // ปุ่มแก้ไขและลบ (สถานะ pending - รออนุมัติ หรือ rejected - ปฏิเสธ)
        if (proposal.status === 'pending' || proposal.status === 'rejected') {
            buttons += `<a href="request-proposal-edit.php?id=${proposal.id}" class="btn btn-icon btn-sm btn-info-transparent rounded-pill" title="แก้ไข"><i class="ri-edit-line"></i></a>`;
            buttons += `<a href="javascript:void(0);" class="btn btn-icon btn-sm btn-danger-transparent rounded-pill" onclick="deleteProposal(${proposal.id})" title="ลบ"><i class="ri-delete-bin-line"></i></a>`;
        }
     ...
     * ส่งคำขอลบ
     */
    async function deleteProposalRequest(id) {
        showLoading();
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                loadProposals(); // โหลดข้อมูลใหม่เพื่อแสดงผลการลบ
            } else {
                showError('ไม่สามารถลบข้อมูลได้: ' + result.message);
            }
        } catch (error) {
            hideLoading();
            handleError(error, 'การลบข้อมูล');
        }
    }
