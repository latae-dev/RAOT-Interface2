(function (window, document) {
  ...
  class InvestmentBudgetApprovalListManager {
    ...
    createTableRow(request, rowNumber) {
      ...
      return `
            <tr class="crm-contact text-center">
                ...
                <td>${this.formatCurrency(this.getItemUnitPrice(request))}</td>
    ...
    getItemQuantity(request) {
      ...
      return Number.isNaN(quantity) ? '0' : quantity.toString();
    }
    ...
    getStatusBadge(request) {
      ...
      if (status === 'submitted') {
        return `<span class="badge rounded-pill ${pendingBadgeClass}">รออนุมัติ</span>`;
      }
