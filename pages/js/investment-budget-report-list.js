...
class InvestmentBudgetReportManager {
  ...
  renderAllTab(data, startIndex) {
    ...
    const rows = data
      .map((item, i) => {
        ...
        const budgetApprovalBadge = this.getBudgetApprovalBadge(item);
                    ...
                    <div class="hstack gap-2 fs-15 justify-content-center">
                        ...
                        ${
                          canBudgetApprove
                            ? `<a href="investment-budget-view-detail.php?id=${
                                item.id
                              }&budgetApproval=1&returnUrl=${encodeURIComponent(
                                window.location.pathname + window.location.search
                              )}" class="btn btn-icon btn-sm btn-info-transparent rounded-pill" title="พิจารณาอนุมัติงบประมาณ"><i class="fe fe-check-circle"></i></a>`
                            : ''
                        }
                    </div>
                </td>
  ...
  getActionButtonsHtml(item) {
    ...
    const viewButton = `<a href="investment-budget-view-detail.php?id=${item.id}&returnUrl=${returnUrl}" class="btn btn-icon btn-sm btn-success-transparent rounded-pill" title="ดูรายละเอียด"><i class="fe fe-eye"></i></a>`;
  ...
  /**
   * Get budget approval status badge HTML
   * Returns badge based on request status and budget approval status
   *
   * @param {Object} request - Request object with status and budget_approval_status
  ...
  getBudgetApprovalBadge(request) {
    ...
    // ถ้ายังไม่ได้รับการอนุมัติจากสำนักงานใหญ่ ให้แสดง "-"
    if (rawStatus !== 'approved') {
      return '<span class="text-muted">-</span>';
    }

    // ถ้าได้รับการอนุมัติจากสำนักงานใหญ่แล้ว แต่ยังไม่มีผลการพิจารณางบประมาณ
  ...
  showError(message) {
    ...
    ['all', 'equipment', 'construction', 'plantation'].forEach((tab) => {
      ...
      if (tbody) {
        const colspan = tab === 'all' ? 9 : tab === 'equipment' ? 15 : 14;
  ...
  async exportProjectReport() {
    ...
    try {
      ...
      allData.forEach((item) => {
        ...
        // ชื่อรายการ - จาก item_name (attachments)
        ...
        // ชื่องบประมาณ - เหมือนกับชื่อรายการ
