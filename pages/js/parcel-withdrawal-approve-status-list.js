...
async function getAll(limit = 10, offset = 0, plan_number = '', start_date = '', end_date = '', status_plan = '') {
    try {
        ...
        const response = await fetch(`../controllers/parcels/plan_controller.php?action=plan_approve_status&doc_type=master&
            limit=${limit}&
            offset=${offset}&
    ...
    try {
        ...
        data.data.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${status_area_thai}</td> <!-- สถานะเขต -->
                <td>${status_head_office_thai}</td> <!-- สถานะ กยท. (สำนักงานใหญ่) -->
                <td>
