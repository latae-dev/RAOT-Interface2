...
$(document).ready(function() {
    ...
    async function loadProposals() {
        ...
        try {
            const currentUser = getCurrentUser();
            const fullUrl = apiUrl + '?action=get_with_details_all&current_user=' + encodeURIComponent(currentUser);
    ...
    function createTableRow(proposal, index) {
        // ตรวจสอบและ normalize status
        const status = proposal.status ? String(proposal.status).toLowerCase().trim() : '';
        ...
        // สร้าง proposal object ที่มี status ที่ normalize แล้ว (ใช้ Object.assign เพื่อความเข้ากันได้)
        const normalizedProposal = Object.assign({}, proposal, { status: status });
            ...
            <tr class="crm-contact text-center">
                ...
                <td>
                    <div class="hstack gap-2 fs-15">
                        ${getActionButtons(normalizedProposal)}
                    </div>
                </td>
            </tr>
        `;
    ...
    function getActionButtons(proposal) {
        ...
        // ปุ่มดู (แสดงเสมอ)
        buttons += `<a href="request-proposal-approvel-view.php?id=${proposal.id}" class="btn btn-icon btn-sm btn-success-transparent rounded-pill" title="ดู"><i class="fe fe-eye"></i></a>`;
        
        // ปุ่มอนุมัติ (สถานะ pending - รออนุมัติ หรือ rejected - ปฏิเสธ)
        // ตรวจสอบ status และแสดงปุ่มอนุมัติสำหรับผู้อนุมัติ (position_id = 11)
        // ตรวจสอบ status จาก proposal object โดยตรง
        const status = (proposal.status || '').toString().toLowerCase().trim();
        
        // แสดงปุ่มอนุมัติเมื่อ status เป็น 'pending' หรือ 'rejected'
        if (status === 'pending' || status === 'rejected') {
            buttons += `<a href="request-proposal-approvel.php?id=${proposal.id}&mode=approve" class="btn btn-icon btn-sm btn-primary-transparent rounded-pill" title="อนุมัติ"><i class="bx bx-list-check"></i></a>`;
        }
    ...
    function getStatusBadge(status) {
        ...
        return STATUS_MAP[normalizedStatus] || '<span class="badge rounded-pill bg-secondary">ไม่ระบุ</span>';
    }
    ...
    // ฟังก์ชันการลบถูกลบออกเพราะหน้า approval ไม่มีการลบ
    ...
    window.updateStatus = async function(id, status) {
        ...
        try {
            const response = await fetch(apiUrl + '?action=update_status&id=' + id, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
            ...
            if (result.status === 'success') {
                ...
                showError('ไม่สามารถอัปเดตสถานะได้: ' + result.message);
            }
        } catch (error) {
            hideLoading();
            handleError(error, 'การอัปเดตสถานะ');
        }
    };
