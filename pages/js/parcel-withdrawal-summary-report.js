...
async function getAll(limit = 10, offset = 0, plan_number = '', start_date = '', end_date = '', province_id = '') {
    try {
        window.showLoading();
        ...
        const response = await fetch(`../controllers/parcels/parcel_report_controller.php?action=parcel_withdrawal_summary_report&doc_type=master&
            limit=${limit}&
            offset=${offset}&
...
async function fetchData(page = 1) {
    try {
        ...
        let province_id = document.querySelector("[name='province_id']")?.value || '';

        const data = await getAll(limit, offset, plan_number, start_date, end_date, province_id);

        const { total_pages: totalPages, current_page: currentPage } = data.pagination;

        departslists(data.departlists);
        ...
        const determineStatuses = (item) => {
            ...
            const status_head_office_thai = getStatusLabel(item.status_head_office);
             ...
             return { status_area_thai, status_province_thai, status_branch_thai, status_head_office_thai };
        };
        ...
        data.data.forEach((item, index) => {
            ...
            const { status_area_thai, status_province_thai, status_branch_thai, status_head_office_thai } = determineStatuses(item);
...
function departslists(dataSelects) {

    // เรียกใช้ select2 สำหรับ select ที่อยู่ใน row ใหม่
    $('#province_id').select2({
        placeholder: "-- ทั้งหมด --",
        allowClear: false,
        data: dataSelects.map(dep => ({
            id: dep.id,  // ใช้ id เป็น id
            text: `${dep.depart_code} - ${dep.depart_name}`  // แสดงทั้ง depart_code และ depart_name ในรายการ
        })),
        templateResult: function (data) {
            // เมื่อแสดงรายการให้แสดง depart_code - depart_name
            return data.text || '';
        },
        templateSelection: function (data) {
            // เมื่อเลือกแสดงแค่ depart_code
            return data.text || '';
        }
    });
}
