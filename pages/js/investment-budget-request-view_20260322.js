class InvestmentBudgetRequestView {
  ...
  createDocumentItem(doc, index) {
    ...
    return `
      <div class="document-item">
        <div class="document-icon ${fileIconInfo.colorClass}">
        ...
        <div class="document-info">
          ...
          <div class="document-meta">
            ${
              fileIconInfo.typeName
                ? `
              <div class="document-meta-item">
                <i class="ri-file-line"></i>
                <span>${fileIconInfo.typeName}${
  ...
  formatBudgetStatusBadge(status) {
    ...
    switch (normalized) {
      case 'approved':
        return '<span class="badge rounded-pill bg-success status-badge">อนุมัติงบประมาณแล้ว</span>';
      case 'rejected':
        return '<span class="badge rounded-pill bg-danger status-badge">ไม่อนุมัติงบประมาณ</span>';
      default:
        return '<span class="badge rounded-pill bg-warning text-dark status-badge">รอพิจารณางบประมาณ</span>';
    }
  }
  ...
  renderBudgetApprovalSection() {
    ...
    const statusBadge = document.getElementById('budget-approval-status-badge');
    if (statusBadge) {
    ...
    console.log('🔍 renderBudgetApprovalSection Debug:', {
      budget_approval_status: request.budget_approval_status,
      hasBudgetDecision: hasBudgetDecision,
      isBudgetApprovalMode: this.isBudgetApprovalMode,
      BUDGET_APPROVAL_MODE_OVERRIDE: window.BUDGET_APPROVAL_MODE_OVERRIDE
    });
    ...
    const isHQUser = canHeadOffice && canInvestment;

    // ตรวจสอบว่า user มีสิทธิ์อนุมัติคำขอนี้หรือไม่
    const canApproveRequest =
      request.can_approve === true ||
      request.can_approve === 'true' ||
      request.can_approve === 1;

    // ซ่อน section จนกว่าสำนักงานใหญ่จะอนุมัติคำขอ
    // ถ้าคำขอยังไม่ได้รับการอนุมัติ ต้องเป็น HQ user เท่านั้นที่เห็น section งบประมาณ
    if (rawStatus !== 'approved') {
      if (!isApprovalMode || !isHQUser) {
    ...
    const shouldShow =
      isApprovalMode ||
      Boolean(request.budget_approval_status) ||
      rawStatus === 'approved';
    ...
    // แต่ถ้ามีการตัดสินใจแล้ว ให้ข้ามไปแสดงแบบ view-only แทน
    if (isApprovalMode && !hasBudgetDecision) {
      ...
      // ถ้า user ไม่ใช่ HQ staff และไม่มีสิทธิ์อนุมัติคำขอนี้ ให้ซ่อนฟอร์ม input และแสดงข้อความแทน
      // แต่ถ้าเป็น HQ user ที่มีสิทธิ์พิจารณางบประมาณ ให้แสดงฟอร์มได้เสมอ
      const canEditBudget = isHQUser || canApproveRequest;

      if (!canEditBudget) {
        // ซ่อน form inputs
        const formInputsContainer = sectionRow.querySelector('.row.gy-3');
        ...
        const cardBody = sectionRow.querySelector('.card-body');
        if (cardBody) {
          let messageDiv = cardBody.querySelector('.non-hq-message');
          if (!messageDiv) {
            messageDiv = document.createElement('div');
            messageDiv.className = 'alert alert-info non-hq-message';
            messageDiv.innerHTML =
              '<i class="bi bi-info-circle me-2"></i>ท่านไม่มีสิทธิ์ในการพิจารณาอนุมัติงบประมาณ';
            cardBody.insertBefore(messageDiv, cardBody.firstChild);
          }
        }

        // แสดงข้อมูลสถานะปัจจุบัน (ถ้ามี)
        this.showBudgetApprovalTextOnlyView(
          request,
      ...
      // ถ้ามีสิทธิ์แก้ไข (HQ user หรือ can_approve) ให้แสดงฟอร์มตามปกติ
      ...
      if (amountInput && request.budget_approved_amount !== undefined) {
        ...
        if (request.budget_approved_amount) {
          amountInput.value = this.formatNumber(request.budget_approved_amount);
        }
      }
      ...
      // เพิ่ม event listener สำหรับ format ตัวเลขแบบมี comma
      if (amountInput && !hasBudgetDecision) {
        this.setupCurrencyInput(amountInput);
      }

      const actions = document.getElementById('budget-approval-actions');
      if (actions) {
        console.log('🎯 Setting actions display:', {
          hasBudgetDecision,
          display: !hasBudgetDecision ? 'flex' : 'none',
          element: actions
        });
        actions.style.display = !hasBudgetDecision ? 'flex' : 'none';
      } else {
        console.warn('⚠️ budget-approval-actions element not found!');
      }
      ...
      if (resultSelect && resultSelect.choicesInstance && hasBudgetDecision) {
    ...
    // ซ่อน input fields และปุ่ม (สำหรับกรณีที่หน้า view detail มี input อยู่)
    const resultSelect = document.getElementById('budget-approval-result');
    const amountInput = document.getElementById('budget-approval-amount');
    const remarkInput = document.getElementById('budget-approval-remark');
    const actions = document.getElementById('budget-approval-actions');
    ...
    // ถ้ามี display elements ให้ซ่อนและใช้ showBudgetApprovalTextOnlyView แทน
    if (resultDisplay || amountDisplay || remarkDisplay) {
      // ซ่อน display elements เดิม
      if (resultDisplay) resultDisplay.style.display = 'none';
      if (amountDisplay) amountDisplay.style.display = 'none';
      if (remarkDisplay) remarkDisplay.style.display = 'none';

      // ซ่อน label/row ของ display elements
      const displayRow = sectionRow.querySelector('.row.gy-3');
      if (displayRow) displayRow.style.display = 'none';

      // ใช้ function แสดงแบบใหม่ที่สวยงาม
      const cardBody = sectionRow.querySelector('.card-body');
      this.showBudgetApprovalTextOnlyView(
        request,
        budgetApprovalStatus,
  ...
  showBudgetApprovalTextOnlyView(request, budgetApprovalStatus, cardBody) {
    if (!cardBody) return;

    console.log('🔍 showBudgetApprovalTextOnlyView:', {
      budgetApprovalStatus,
      fiscal_year: request.fiscal_year,
      fiscal_year_id: request.fiscal_year_id,
      budget_approved_amount: request.budget_approved_amount,
      budget_approval_remark: request.budget_approval_remark
    });

    // ลบข้อความเก่า (ถ้ามี)
    const existingView = cardBody.querySelector('.budget-text-only-view');
    if (existingView) {
      existingView.remove();
    }

    // สร้าง container สำหรับแสดงข้อมูลแบบใหญ่ตรงกลาง
    const viewContainer = document.createElement('div');
    viewContainer.className = 'budget-text-only-view text-center py-5';

    let statusHTML = '';

    // แสดงข้อความตามสถานะ
    if (!budgetApprovalStatus || budgetApprovalStatus === 'pending') {
      // กรณี pending แสดงข้อความรอพิจารณา
      statusHTML = `
        <div class="mb-4">
          <i class="ri-time-line" style="font-size: 4rem; color: #ffc107;"></i>
        </div>
        <div>
          <span class="badge rounded-pill bg-warning text-dark status-badge mb-3">
            อยู่ระหว่างรอผลการพิจารณางบประมาณ
          </span>
        </div>
        <p class="text-muted mt-3">ระบบจะแสดงข้อมูลเมื่อได้รับการอนุมัติงบประมาณ</p>
      `;
    } else if (budgetApprovalStatus === 'approved') {
      // กรณีอนุมัติแล้ว
      let canShowBudget = true; // Default: แสดงข้อมูลได้

      // ใช้ fiscal_year_id แทน fiscal_year
      const fiscalYearValue = request.fiscal_year_id || request.fiscal_year;

      // ถ้ามี fiscal_year ให้เช็คว่าถึงปีงบประมาณหรือยัง
      if (fiscalYearValue) {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear() + 543; // แปลงเป็น พ.ศ.
        const currentMonth = currentDate.getMonth() + 1; // 1-12
        const fiscalYear = parseInt(fiscalYearValue);

        // ปีงบประมาณเริ่ม 1 ตุลาคม ของปีก่อนหน้า
        // เช่น งบ 2568 เริ่ม 1 ตุลาคม 2567
        const fiscalStartYear = fiscalYear - 1; // ปีที่เริ่มต้นปีงบประมาณ
        const fiscalStartMonth = 10; // ตุลาคม

        // เช็คว่าถึงเดือนตุลาคมของปีที่เริ่มต้นปีงบประมาณหรือยัง
        canShowBudget = false;
        if (currentYear > fiscalStartYear) {
          canShowBudget = true;
        } else if (
          currentYear === fiscalStartYear &&
          currentMonth >= fiscalStartMonth
        ) {
          canShowBudget = true;
        }

        console.log('📅 Fiscal Year Check:', {
        ...
        if (!canShowBudget) {
          ...
          statusHTML = `
            <div class="mb-4">
              <i class="ri-calendar-check-line" style="font-size: 4rem; color: #28a745;"></i>
            </div>
            <div>
              <span class="badge rounded-pill bg-success status-badge mb-3">
                งบประมาณได้รับการอนุมัติแล้ว
              </span>
            </div>
            <div class="mt-4 mx-auto" style="max-width: 600px;">
              <p class="text-muted fs-5 mb-3">
                งบประมาณของท่านได้รับการอนุมัติแล้ว
              </p>
              <p class="text-primary fs-6">
                <i class="ri-information-line me-1"></i>
                ข้อมูลรายละเอียดงบประมาณจะแสดงให้ท่านเห็นเมื่อถึง<strong>ปีงบประมาณ ${fiscalYear}</strong><br>
                <small class="text-muted">(เริ่มตั้งแต่เดือนตุลาคม ${fiscalStartYear} เป็นต้นไป)</small>
              </p>
            </div>
          `;
        }
      ...
      if (canShowBudget) {
        // แสดงข้อมูล (ถ้าไม่มี fiscal_year หรือถึงปีงบประมาณแล้ว)
        statusHTML = `
          <div class="mb-4">
            <i class="ri-checkbox-circle-line" style="font-size: 4rem; color: #28a745;"></i>
          </div>
          <div>
            <span class="badge rounded-pill bg-success status-badge mb-3">อนุมัติ</span>
          </div>
        `;

        // แสดงจำนวนเงินที่อนุมัติ
        if (
          ...
          statusHTML += `
            <div class="mt-4 mb-3">
              <h5 class="text-muted mb-2">งบประมาณที่อนุมัติ</h5>
              <h3 class="text-success fw-bold">${this.formatCurrencyDisplay(
                request.budget_approved_amount
              )}</h3>
            </div>
          `;
        ...
        // แสดงหมายเหตุ (ถ้ามี)
        if (request.budget_approval_remark) {
          statusHTML += `
            <div class="mt-4 text-start mx-auto" style="max-width: 600px;">
              <h6 class="text-muted mb-2">หมายเหตุ</h6>
              <p class="border rounded p-3 bg-light">${request.budget_approval_remark}</p>
            </div>
          `;
        }

        // แสดงข้อมูลผู้อนุมัติและวันที่ (ถ้ามี)
        if (request.budget_approved_at) {
          const approver =
          ...
          statusHTML += `
            <div class="text-muted small mt-4">
              ล่าสุดโดย ${approver} เมื่อ ${this.formatDate(
    ...
    } else if (budgetApprovalStatus === 'rejected') {
      statusHTML = `
        <div class="mb-4">
          <i class="ri-close-circle-line" style="font-size: 4rem; color: #dc3545;"></i>
        </div>
        <div>
          <span class="badge rounded-pill bg-danger status-badge mb-3">ไม่อนุมัติ</span>
        </div>
      `;
      ...
      if (request.budget_approval_remark) {
        statusHTML += `
          <div class="mt-4 text-start mx-auto" style="max-width: 600px;">
            <h6 class="text-muted mb-2">เหตุผล</h6>
            <p class="border rounded p-3 bg-light">${request.budget_approval_remark}</p>
          </div>
        `;
      }
    ...
    viewContainer.innerHTML = statusHTML;
  ...
  async submitBudgetApproval(action) {
    ...
    try {
      ...
      // ลบ comma ออกก่อนส่งค่าไปยัง server
      const rawAmount =
        amountInput?.dataset?.rawValue ||
        amountInput?.value?.replace(/,/g, '') ||
        '';
      formData.append('approved_amount', rawAmount);
  ...
  handleApprovalActionChange() {
    ...
    const approvalSelect = document.getElementById('approval-action');

    if (!commentsRequired) {
      return;
    }

    if (approvalSelect && approvalSelect.value === 'reject') {
  ...
  async handleApprovalSubmit() {
    ...
    const approvalSelect = document.getElementById('approval-action');
    const action = approvalSelect ? approvalSelect.value : '';
    if (!action) {
    ...
    try {
      ...
      const isApprovePage = currentPath.includes(
        'investment-budget-approve.php'
      );
      ...
      // Render ใหม่เพื่อซ่อนปุ่มและแสดงข้อมูลที่บันทึก (หลังจาก Swal ปิดแล้ว)
      this.renderBudgetApprovalSection();

      // ถ้าอยู่ในหน้า approve list ให้ redirect กลับไปหน้า list
      if (isApprovePage) {
        window.location.href = 'investment-budget-approvel-list.php';
      }
      // ถ้าอยู่ในหน้า view detail ให้อยู่ที่หน้าเดิม (ปุ่มจะหายไปแล้วจากการ render ใหม่)
      return;
  ...
  formatNumber(number) {
    ...
    return InvestmentBudgetCommon.formatNumber(number);
  }
  ...
  renderReturnBanner(request) {
    ...
    if (!banner) {
      const actionSection = document.getElementById('approval-action-section');
      const referenceNode =
        actionSection || document.getElementById('approval-permission-section');
  ...
  parseBooleanFlag(value) {
    ...
    return ['1', 'true', 't', 'yes', 'y', 'active'].includes(normalized);
  }
  ...
  /**
   * Setup currency input formatting for integer budget amounts (no decimals)
   * Real-time formatting with comma separators and proper cursor positioning
   *
  ...
  setupCurrencyInput(inputElement) {
    ...
    if (inputElement.dataset.currencySetup === 'true') return;
    inputElement.dataset.currencySetup = 'true';

    // Change to text input if needed
    ...
    if (inputElement.value) {
      const numValue = parseFloat(inputElement.value.replace(/,/g, ''));
      if (!isNaN(numValue)) {
        inputElement.value = InvestmentBudgetCommon.formatNumber(numValue);
      }
    }

    // Event: Input - Format แบบ real-time พร้อมรักษาตำแหน่ง cursor
    inputElement.addEventListener('input', (e) => {
      const cursorPosition = e.target.selectionStart;
      const oldValue = e.target.value;

      // กรองเฉพาะตัวเลข (integer only, no decimals)
      ...
      if (cleaned) {
        ...
        if (!isNaN(numValue)) {
          formatted = InvestmentBudgetCommon.formatNumber(numValue);
        }
      }
      ...
      // นับจำนวนตัวเลขก่อน cursor เดิม
      const oldBeforeCursor = oldValue.substring(0, cursorPosition);
      const digitsBeforeCursor = oldBeforeCursor.replace(/[^0-9]/g, '').length;

      // หาตำแหน่งใหม่ที่มีจำนวนตัวเลขเท่ากัน
      ...
      // ถ้านับตัวเลขไม่ครบ ให้วาง cursor ที่ท้าย
      if (digitCount < digitsBeforeCursor) {
      ...
      try {
        ...
        // Ignore error if input doesn't support selection range
      }

      // เก็บค่าดิบไว้ใน data attribute
      e.target.dataset.rawValue = cleaned;
    });

    // Event: Blur - Validate และ format สุดท้าย
    inputElement.addEventListener('blur', (e) => {
      ...
      if (rawValue) {
        ...
        if (!isNaN(numValue) && numValue > 0) {
          e.target.value = InvestmentBudgetCommon.formatNumber(numValue);
          e.target.dataset.rawValue = numValue.toString();
        } else {
          e.target.value = '';
