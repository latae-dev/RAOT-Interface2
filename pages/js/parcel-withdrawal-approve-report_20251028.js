...
async function getAll(limit = 10, offset = 0, plan_number = '', start_date = '', end_date = '', status_plan = '') {
    try {
        ...
        const response = await fetch(`../controllers/parcels/parcel_report_controller.php?action=parcel_withdrawal_approve_report&doc_type=master&
            limit=${limit}&
            offset=${offset}&
...
async function fetchData(page = 1) {
    try {
        ...
        const determineStatuses = (item) => {
            ...
            const status_doc_thai = getStatusLabel(item.status_doc);

             return { status_area_thai, status_province_thai, status_branch_thai, status_head_office_thai, status_doc_thai };
        };
        ...
        data.data.forEach((item, index) => {
            ...
            const { status_area_thai, status_province_thai, status_branch_thai, status_head_office_thai, status_doc_thai } = determineStatuses(item);
            ...
            row.innerHTML = `
                ...
                <td>${convertToBuddhistEra(item.doc_start)}</td>
                <td>${item.doc_number}</td>
                <td>${item.doc_form_name}</td>
                <td>${item.user_requ_name}</td>
                <td>${status_doc_thai}</td>
            `;
