  ...
  async loadFormData(data) {
    ...
    try {
      ...
      if (data.version_plan_id) {
        ...
        if (window.userHierarchyData && window.userHierarchyData.allowed_version_plan_id) {
          await this.masterData.setValue(
            'version-plan-select',
            window.userHierarchyData.allowed_version_plan_id
          );
          console.log('✅ Set default version plan from user hierarchy:', window.userHierarchyData.allowed_version_plan_id);
        }
      }
