(function (window, document) {
  ...
  class InvestmentBudgetListManager {
    ...
    createTableRow(request, rowNumber) {
      ...
      return `
            <tr class="crm-contact text-center">
                ...
                <td>${request.budget_source_name_full || '-'}</td>
                ...
                <td>${this.formatCurrency(
                  this.getFirstItemUnitPrice(request)
                )}</td>
                <td>${this.formatCurrency(request.total_amount || 0)}</td>
    ...
    getStatusBadge(request) {
      ...
      if (rawStatus.startsWith('pending_')) {
        ...
        return `<span class="badge rounded-pill bg-info text-white">${text}</span>`;
      }
      ...
      return `<span class="badge rounded-pill ${config.class}">${config.text}</span>`;
    }
    ...
    getFirstItemQuantity(request) {
      ...
      return Number.isNaN(quantity) ? '-' : quantity.toString();
    }
  ...
  function viewRequest(id) {
    ...
    window.location.href = `investment-budget-view-detail.php?id=${id}`;
  }
