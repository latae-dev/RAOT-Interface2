...
async function loadMrdGroups() {
    try {
        ...
        if (result && result.status === 'success' && Array.isArray(result.data)) {
            ...
            window.mrdGroupData = result.data.filter(item => !item.code || !item.code.startsWith('A'));
        }
    } catch (err) {
        // ไม่แสดง error เพื่อไม่รบกวนผู้ใช้
    }
...
async function loadExpenPr() {
    try {
        ...
        if (result && result.status === 'success' && Array.isArray(result.data)) {
            ...
            window.expenPrData = result.data.filter(item => !item.code || !item.code.startsWith('A'));
        }
    } catch (err) {
        // ไม่แสดง error เพื่อไม่รบกวนผู้ใช้
    }
...
function collectProposalData() {
    ...
    // รวมข้อมูล (ตั้งค่า business_type_id หลังสุดเพื่อ override ค่าจาก sectionData)
    return {
        ...formData,
        ...sectionData,
        ...totals,
