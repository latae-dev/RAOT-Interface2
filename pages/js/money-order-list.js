...
(async () => {
    try {
        ...
        function displayData(data, pageOffset = 0) {
            ...
            data.forEach((item, index) => {
                const statusLabels = {
                    ...
                    audit_reject: `<span class="badge rounded-pill bg-dark">กองตรวจจ่าย ไม่อนุมัติ</span>`
                };
                ...
                const canEditOrDelete = item.req_user_code === params_all.current_user.user_code && (item.approve_status === 'waiting' || item.approve_status === 'finance_reject' || item.approve_status === 'audit_reject');
                // ✅ ถ้า st_hf === 'approve' ไม่ใส่ disabled
                const disabledAttr = item.st_hf === 'approve' ? '' : 'disabled';
                const linkEdit = `money-order-edit.php?id=${item.id}`;
                const linkView = `money-order-view.php?id=${item.id}&is_approve=0&is_action=view`;
                const linkExport = `./export/money-order-list-export.php?id=${item.id}`;
                ...
                if (deleteBtn) {
                    deleteBtn.addEventListener('click', (e) => {
                        e.preventDefault();
                        Swal.fire({
                            title: `รหัสคำขอใบสั่งจ่าย : ${item.compensation_number}`,
        ...
        async function deleteStatus(id) {
            try {
                window.showLoading();
                const response = await fetch(`../controllers/withdraws/wrd_compensation_controller.php?action=delete_status&id=${id}`, {
                    method: 'DELETE'
                });
        ...
        // ฟังก์ชันค้นหา
        async function getSearch(limit = 10, offset = 0, doc_number = '', start_date = '', end_date = '', status = '', key = '') {
            try {
                window.showLoading();
                const response = await fetch(`../controllers/withdraws/wrd_compensation_controller.php?
                    action=compensation_search&
                    key=${key}&
                    limit=${limit}&
                    offset=${offset}&
        ...
        async function searchAndDisplay(page = 1) {
            try {
                ...
                const result = await getSearch(currentLimit, pageOffset, docNumber, startDate, endDate, status, 'approve_status');
