...
async function createData() {
    ...
    document.querySelectorAll("#table-body tr").forEach(row => {
        ...
        if(row_deli_date.value == ''){
            $(row_deli_date).addClass('border-danger');
            checked = false;
        }else{
            $(row_deli_date).removeClass('border-danger');
        }

        let row_quantity = row.querySelector('input[name="quantity"]');
        if(row_quantity.value == 0){
            $(row_quantity).addClass('border-danger');
            checked = false;
        }else{
            $(row_quantity).removeClass('border-danger');
        }
        if(row_quantity.value < row.querySelector('input[name="qty_min"]').value || row_quantity.value > row.querySelector('input[name="qty_max"]').value){
    ...
    if(!checked_quantity){
        Swal.fire({
            text: "กรุณาตรวจสอบ ขั้นต่ำในการเบิก 10 และสูงสุด 100 !!!",
            icon: "warning"
        });
        return;
    }
