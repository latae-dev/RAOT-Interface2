...
class BusinessBudgetModel {
    ...
    public function insertActivityBudgets($budget_id, $activities) 
    {
        ...
        foreach ($activities as $key => $activity) {
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $cleanedQuantity    = cleanNumberForDB($quantity);
                $cleanedPrice       = cleanNumberForDB($price);
                $cleanedAmount      = cleanNumberForDB($amount);
                $cleanedTotalAmount = cleanNumberForDB($totalAmount);
        ...
        foreach ($expenses as $key => $expense) {
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $obj = [
                    ...
                    $planTrip,
                    $officer,
                    $people         !== null && $people         !== '' ? cleanNumberForDB($people)  : null,
                    $workDays       !== null && $workDays       !== '' ? cleanNumberForDB($workDays)  : null,
                    $allowanceRate  !== null && $allowanceRate  !== '' ? cleanNumberForDB($allowanceRate)  : null,
                    $allowanceAmount!== null && $allowanceAmount!== '' ? cleanNumberForDB($allowanceAmount): null,
                    $transportRate  !== null && $transportRate  !== '' ? cleanNumberForDB($transportRate)  : null,
                    $transportAmount!== null && $transportAmount!== '' ? cleanNumberForDB($transportAmount): null,
                    $stayDays       !== null && $stayDays       !== '' ? cleanNumberForDB($stayDays)       : null,
                    $stayRate       !== null && $stayRate       !== '' ? cleanNumberForDB($stayRate)       : null,
                    $stayAmount     !== null && $stayAmount     !== '' ? cleanNumberForDB($stayAmount)     : null,
                    $fuelTrips      !== null && $fuelTrips      !== '' ? cleanNumberForDB($fuelTrips)      : null,
                    $fuelRate       !== null && $fuelRate       !== '' ? cleanNumberForDB($fuelRate)       : null,
                    $fuelAmount     !== null && $fuelAmount     !== '' ? cleanNumberForDB($fuelAmount)     : null,
                    $totalActivity  !== null && $totalActivity  !== '' ? cleanNumberForDB($totalActivity)  : null
                ];
    ...
    public function insertOperationBudgets($budget_id, $operations)
    {
        ...
        foreach ($operations as $key => $operation) {
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $weight       = isset($operation['weight'][$i]) ? cleanNumberForDB($operation['weight'][$i]) : null;
                $target       = isset($operation['target'][$i]) ? $operation['target'][$i] : null;
                $unit         = isset($operation['unit'][$i]) ? $operation['unit'][$i] : null;
                $oct          = isset($operation['oct'][$i]) ? cleanNumberForDB($operation['oct'][$i]) : null;
                $nov          = isset($operation['nov'][$i]) ? cleanNumberForDB($operation['nov'][$i]) : null;
                $dec          = isset($operation['dec'][$i]) ? cleanNumberForDB($operation['dec'][$i]) : null;
                $jan          = isset($operation['jan'][$i]) ? cleanNumberForDB($operation['jan'][$i]) : null;
                $feb          = isset($operation['feb'][$i]) ? cleanNumberForDB($operation['feb'][$i]) : null;
                $mar          = isset($operation['mar'][$i]) ? cleanNumberForDB($operation['mar'][$i]) : null;
                $apr          = isset($operation['apr'][$i]) ? cleanNumberForDB($operation['apr'][$i]) : null;
                $may          = isset($operation['may'][$i]) ? cleanNumberForDB($operation['may'][$i]) : null;
                $jun          = isset($operation['jun'][$i]) ? cleanNumberForDB($operation['jun'][$i]) : null;
                $jul          = isset($operation['jul'][$i]) ? cleanNumberForDB($operation['jul'][$i]) : null;
                $aug          = isset($operation['aug'][$i]) ? cleanNumberForDB($operation['aug'][$i]) : null;
                $sep          = isset($operation['sep'][$i]) ? cleanNumberForDB($operation['sep'][$i]) : null;
    ...
    public function createProject($projectName)
    {
        ...
        $query = "
            INSERT INTO ".PARAMETERS_TB_PROJECTS." (".implode(',', array_keys($obj)).")
        ...
        if (!$result) {
            arrx($result);
            throw new Exception("Insert ".PARAMETERS_TB_PROJECTS." failed");
        }
    ...
    public function createProjectActivity($projectCode, $activityName)
    {
        $obj['project_code'] = $projectCode;
        $obj['activity_code'] = '100400001';
        ...
        $query = "
            INSERT INTO ".PARAMETERS_TB_PROJECTS_ACTIVITIES." (".implode(',', array_keys($obj)).")
            VALUES (".implode(',', $placeholders).");
        ";
        ...
        if (!$result) {
            arrx($result);
            throw new Exception("Insert ".PARAMETERS_TB_PROJECTS_ACTIVITIES." failed");
        }

        $row = pg_fetch_assoc($result);
        return $obj['activity_code'];
    }
    ...
    public function insertBusinessBudget($data) 
    {
        ...
        try {
            if($data['project_type_name'] == 2 && $data['action'] == 'waiting') {
                $projectCode = $this->createProject($data['project_name']);
                $activityCode = $this->createProjectActivity($projectCode, $data['activity_name']);
        ...
        try {
            ...
            foreach ($allowedPositions as $pos) {
                if ($pos === 'APPRP') {
                    ...
                    $geoConditions[] = "(u.branch_id = ".$user['branch_id'].")";
                    $approveConditions[] = "(bb.approve_position_code = 'APBR' AND " . end($geoConditions) . ")";
                } elseif ($pos === 'APPV') {
                    $geoConditions[] = "(u.province_id = (select province_id from ".USERS_TB_USERS." where id = '".$user['id']."') or u.branch_id IN (
                        select tb.id 
                        from ".USERS_TB_USERS." u
                        join ".USERS_TB_PROVINCES." tp on u.province_id = tp.id
                        join ".USERS_TB_BRANCHS." tb on tb.province_id = tp.id
                        where u.id = '".$user['id']."'
                    ))";
                    $approveConditions[] = "(bb.approve_position_code = 'APPV' AND " . end($geoConditions) . ")";
                } elseif ($pos === 'APAR') {
                    $geoConditions[] = "(u.areas_id = (select areas_id from ".USERS_TB_USERS." where id = '".$user['id']."') or u.province_id IN (
                        select tp.id 
                        from ".USERS_TB_USERS." u
                        join ".USERS_TB_PROVINCES." tp on tp.area_id = u.areas_id
                        where u.id = '".$user['id']."'
                    ) or u.branch_id IN (
                        select tb.id 
                        from ".USERS_TB_USERS." u
    ...
    public function reject($post)
    {
        ...
        try {
            ...
            if($post['approve_status'] == 'rejected') {
                $objUpdate['status'] = 'rejected';
                $objUpdate['approve_position_code'] = null;
                $objUpdate['responsibilities_approves'] = null;
            }

            if(isset($post['approve_position_code']) && $post['approve_position_code']) {
                $objUpdate['approve_position_code'] = $post['approve_position_code'];
            }
