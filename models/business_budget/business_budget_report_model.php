...
class BusinessBudgetReportModel {
    ...
    public function getBusinessReportList($param = [])
    {
        try {
            ...
            if (!empty($param['export_type']) && $param['export_type'] == 4) {
                $conditions[] = "tbb.sap_status = 'not_synced' ";
            }

            if (!empty($param['sap_status'])) {
                $conditions[] = "tbb.sap_status = '".$param['sap_status']."' ";
            }
        ...
        try {
            ...
            if ($result) {
                ...
                foreach($rows as $key => $rs) {
                    ...
                    $rows[$key]['activity'] = $this->getActivityCode($rs['id']);
                }
    ...
    public function exportSap($param = []) 
    {
        try {
            $user = $_SESSION['user_data'];
            $params = [];
            ...
            $query = "
                select 
                    ...
                    tpa.activity_code,
                    (case when tpa.activity_name is not null then tpa.activity_name
                        else tbb.activity_name
                    ...
                    (select (sum(allowance_amount) + sum(transport_amount) + sum(stay_amount) + sum(fuel_amount))  FROM ".BUDGETS_TB_EXPENSES_BUDGETS." where business_budget_id = tbb.id) as total_expenses_amount
                from ".BUDGETS_TB_BUSINESS_BUDGETS." tbb 
                left join ".BUDGETS_TB_ACTIVITIES." act on tbb.id = act.business_budget_id
                left join ".PARAMETERS_TB_PROJECTS_ACTIVITIES." tpa on tpa.activity_code = act.activity_code 
                left join ".PARAMETERS_TB_BUDGET_SOURCES." tbs on tbs.id = tbb.budget_source_code 
                left join users.tb_users tu on tu.id = tbb.created_by 
                left join users.tb_departs td on td.id = tu.depart_id
                join users.tb_branchs b ON b.id = tu.branch_id
    ...
    public function exportSap2($param = []) 
    {
        try {
            $user = $_SESSION['user_data'];
            $params = [];
            ...
            $query = "
                select 
                    ...
                    tbb.budget_source_code,
                    td.depart_code,
                    (select sum(total_amount) FROM ".BUDGETS_TB_ACTIVITY_BUDGETS." where business_budget_id = tbb.id and act.seq = seq) as total_activity_amount,
                    (select (sum(allowance_amount) + sum(transport_amount) + sum(stay_amount) + sum(fuel_amount))  FROM ".BUDGETS_TB_EXPENSES_BUDGETS." where business_budget_id = tbb.id and act.seq = seq) as total_expenses_amount
                from ".BUDGETS_TB_BUSINESS_BUDGETS." tbb 
                left join ".BUDGETS_TB_ACTIVITIES." act on tbb.id = act.business_budget_id
                left join ".PARAMETERS_TB_PROJECTS_ACTIVITIES." tpa on tpa.activity_code = act.activity_code 
                left join ".PARAMETERS_TB_BUDGET_SOURCES." tbs on tbs.id = tbb.budget_source_code 
    ...
    public function syncToSap($ids)
    {
        ...
        $query = "
            ...
            SET sap_status = 'synced',
            updated_at = '".convertDateTime()."',
            updated_by = '".$user['id']."'
            WHERE id IN ($inClause)
        ";

        $result = pg_query_params($this->conn, $query, $ids);

        if ($result === false) {
            throw new Exception('Failed to update sap_status');
        }
    ...
    {
        try {
            $query = "SELECT A.*, B.activity_name FROM ".BUDGETS_TB_ACTIVITIES." A
                        join ".PARAMETERS_TB_PROJECTS_ACTIVITIES." B on B.activity_code = A.activity_code
                        where A.business_budget_id = $1 ORDER BY A.seq ASC";
            $result = pg_query_params($this->conn, $query, [$businessBudgetId]);
