...
class IbRequestsModel
{
    ...
    private $reportFieldColumns = [];
    private $budgetFieldsChecked = false;
    private $budgetFieldsAvailable = false;
    private $lastError = '';
    ...
    private function ensureReportFieldSupport()
    {
        ...
        $this->reportFieldColumns = $found;
        $this->reportFieldsAvailable = count($found) > 0;
        return $this->reportFieldsAvailable;
    }

    private function hasReportFieldColumn($column)
    {
        return in_array($column, $this->reportFieldColumns, true);
    }

    private function ensureBudgetFieldSupport()
    {
        if ($this->budgetFieldsChecked) {
            return $this->budgetFieldsAvailable;
        }

        $this->budgetFieldsChecked = true;

        $columns = [
            'budget_approval_status',
            'budget_approved_amount',
            'budget_approval_remark',
            'budget_approved_by',
            'budget_approved_at'
        ];
        ...
        $result = pg_query($this->conn, $query);
        if (!$result) {
            $this->budgetFieldsAvailable = false;
            return false;
        }
        ...
        $this->budgetFieldsAvailable = count(array_intersect($columns, $found)) === count($columns);
        return $this->budgetFieldsAvailable;
    }
        ...
        try {
            ...
            $query = "INSERT INTO " . $this->table . "
                     (request_number, fiscal_year_id, version_plan_id, asset_type_id,
                      ...
                      multi_year_disbursement, work_period, work_period_number, attachment_files,
        ...
        try {
            ...
            $query = "UPDATE " . $this->table . "
                     SET fiscal_year_id = $1, version_plan_id = $2, asset_type_id = $3,
                         ...
                         work_period = $25, work_period_number = $26, attachment_files = $27,
                         request_number = COALESCE($28, request_number),
                         functional_area = $29,
    ...
    public function updateReportFields($id, array $fields, $updatedBy = null)
    {
        ...
        $allowed = ['equipment_code_type_id', 'equipment_type_id', 'count_unit_id', 'account_code', 'fund_area_id'];
        $setClauses = [];
        $params = [];
        $index = 1;
        ...
        $needsRecalculateFunctionalArea = (
            ($this->hasReportFieldColumn('equipment_type_id') && array_key_exists('equipment_type_id', $fields)) ||
            ($this->hasReportFieldColumn('equipment_code_type_id') && array_key_exists('equipment_code_type_id', $fields))
        );
        ...
        $needsResetSapStatus = (
            ($this->hasReportFieldColumn('equipment_code_type_id') && array_key_exists('equipment_code_type_id', $fields)) ||
            ($this->hasReportFieldColumn('equipment_type_id') && array_key_exists('equipment_type_id', $fields)) ||
            ($this->hasReportFieldColumn('count_unit_id') && array_key_exists('count_unit_id', $fields)) ||
            ($this->hasReportFieldColumn('fund_area_id') && array_key_exists('fund_area_id', $fields))
        );
    ...
    public function readRequest($id)
    {
        ...
        if ($this->ensureReportFieldSupport()) {
            ...
            if ($this->hasReportFieldColumn('equipment_code_type_id')) {
                $reportSelectParts[] = 'r.equipment_code_type_id';
                $reportSelectParts[] = 'ect.code AS equipment_code_type_code';
                $reportSelectParts[] = 'ect.name AS equipment_code_type_name';
                $reportJoinParts[] = "LEFT JOIN parameters.tb_equipment_code_type ect ON r.equipment_code_type_id = ect.id";
            } else {
                $reportSelectParts[] = 'NULL::INTEGER AS equipment_code_type_id';
                $reportSelectParts[] = 'NULL::VARCHAR AS equipment_code_type_code';
                $reportSelectParts[] = 'NULL::VARCHAR AS equipment_code_type_name';
            }

            if ($this->hasReportFieldColumn('equipment_type_id')) {
                $reportSelectParts[] = 'r.equipment_type_id';
                $reportSelectParts[] = 'et.code AS equipment_type_code';
                $reportSelectParts[] = 'et.name AS equipment_type_name';
                $reportJoinParts[] = "LEFT JOIN parameters.tb_equipment_type et ON r.equipment_type_id = et.id";
            } else {
                $reportSelectParts[] = 'NULL::INTEGER AS equipment_type_id';
                $reportSelectParts[] = 'NULL::VARCHAR AS equipment_type_code';
                $reportSelectParts[] = 'NULL::VARCHAR AS equipment_type_name';
            }

            if ($this->hasReportFieldColumn('count_unit_id')) {
                $reportSelectParts[] = 'r.count_unit_id';
                $reportSelectParts[] = 'cu.count_unit_name AS count_unit_name';
                $reportSelectParts[] = 'cu.count_unit_code AS count_unit_code';
                $reportJoinParts[] = "LEFT JOIN parameters.tb_count_units cu ON r.count_unit_id = cu.id";
            } else {
                $reportSelectParts[] = 'NULL::INTEGER AS count_unit_id';
                $reportSelectParts[] = 'NULL::VARCHAR AS count_unit_name';
                $reportSelectParts[] = 'NULL::VARCHAR AS count_unit_code';
            }

            if ($this->hasReportFieldColumn('account_code')) {
                $reportSelectParts[] = 'r.account_code';
            } else {
                $reportSelectParts[] = 'NULL::VARCHAR AS account_code';
            }

            if ($this->hasReportFieldColumn('fund_area_id')) {
                $reportSelectParts[] = 'r.fund_area_id';
                $reportSelectParts[] = 'fa.code AS fund_area_code';
                $reportSelectParts[] = 'fa.name AS fund_area_name';
                $reportJoinParts[] = "LEFT JOIN parameters.tb_ib_fund_area fa ON r.fund_area_id = fa.id";
            } else {
                $reportSelectParts[] = 'NULL::INTEGER AS fund_area_id';
                $reportSelectParts[] = 'NULL::VARCHAR AS fund_area_code';
                $reportSelectParts[] = 'NULL::VARCHAR AS fund_area_name';
            }

            $reportSelect = ",\n                         " . implode(",\n                         ", $reportSelectParts);
            $reportJoins = !empty($reportJoinParts) ? "\n                  " . implode("\n                  ", $reportJoinParts) : '';
        } else {
            $reportSelect = ",
                         ...
                         NULL::VARCHAR AS fund_area_code,
                         NULL::VARCHAR AS fund_area_name";
        }
        ...
        $query = "SELECT r.*,
                         ...
                         CONCAT(bs.code,' ', bs.name) AS budget_source_name_full,
                         ...
                         COALESCE(b_direct.id, b_via_branch.id) AS branch_id,
                         COALESCE(b_direct.branch_code, b_via_branch.branch_code) AS branch_code,
                         COALESCE(b_direct.branch_name, b_via_branch.branch_name) AS branch_name,
                         COALESCE(pv_direct.id, pv_via_branch.id) AS province_id,
                         COALESCE(pv_direct.province_code, pv_via_branch.province_code) AS province_code,
                         COALESCE(pv_direct.province_name, pv_via_branch.province_name) AS province_name,
                         COALESCE(ar_direct.id, ar_via_province.id) AS area_id,
                         COALESCE(ar_direct.area_code, ar_via_province.area_code) AS area_code,
                         COALESCE(ar_direct.area_name, ar_via_province.area_name) AS area_name,
                         COALESCE(ho_direct.id, ho_via_area.id) AS head_office_id,
                         COALESCE(ho_direct.head_office_code, ho_via_area.head_office_code) AS head_office_code,
                         COALESCE(ho_direct.head_office_name, ho_via_area.head_office_name) AS head_office_name" . $reportSelect . "
                  FROM " . $this->table . " r
                  ...
                  LEFT JOIN users.tb_branchs b_via_branch ON b_via_branch.id = u.branch_id
                  LEFT JOIN users.tb_provinces pv_via_branch ON pv_via_branch.id = b_via_branch.province_id
                  LEFT JOIN users.tb_areas ar_via_province ON ar_via_province.id = pv_via_branch.area_id
                  LEFT JOIN users.tb_head_offices ho_via_area ON ho_via_area.id = ar_via_province.head_office_id
                  LEFT JOIN users.tb_branchs b_direct ON b_direct.branch_code = d.depart_code
                  LEFT JOIN users.tb_provinces pv_direct ON pv_direct.province_code = d.depart_code
                  LEFT JOIN users.tb_areas ar_direct ON ar_direct.area_code = d.depart_code
                  LEFT JOIN users.tb_head_offices ho_direct ON ho_direct.head_office_code = d.depart_code
    ...
    /**
     * บันทึกผลการพิจารณาอนุมัติงบประมาณ
     */
    public function saveBudgetApproval($requestId, $action, $amount, $remark, $userId)
    {
        try {
            pg_query($this->conn, 'BEGIN');

            // ปิด active เดิม (ถ้ามี)
            $deactivate = pg_query_params(
                $this->conn,
                'UPDATE investment_budget.tb_ib_budget_approvals SET is_active = FALSE WHERE request_id = $1',
                [$requestId]
            );
            if ($deactivate === false) {
                throw new Exception('Deactivate old budget approval failed');
            }

            // แทรกประวัติใหม่
            $insert = pg_query_params(
                $this->conn,
                'INSERT INTO investment_budget.tb_ib_budget_approvals
                  (request_id, action, approved_amount, remark, approved_by, approved_at, is_active)
            ...
            if ($insert === false) {
                throw new Exception('Insert budget approval failed');
            }

            // อัปเดต summary ใน tb_ib_requests
            $update = pg_query_params(
                $this->conn,
                'UPDATE investment_budget.tb_ib_requests
                 SET budget_approval_status = $2,
                     budget_approved_amount = $3,
                     budget_approval_remark = $4,
            ...
            if ($update === false) {
                throw new Exception('Update request budget approval failed');
            }
    ...
    public function readRequests($filters = [], $pagination = null)
    {
        ...
        if (isset($filters['status']) && $filters['status'] !== '') {
            ...
            $includeReturned = !empty($filters['include_returned']);

            if ($includeReturned && $statusValue === 'rejected') {
                $param_count++;
                $where_conditions[] = "(r.status = $" . $param_count . " OR r.is_returned = TRUE)";
                $params[] = $statusValue;
            } else {
                $param_count++;
            ...
            unset($filters['include_returned']);
        }
        ...
        if ($approvalScope !== null && is_array($approvalScope)) {
            $scopeConditions = [];
            // ใช้ COALESCE เพื่อ check ทั้ง 2 ทาง (via branch และ via depart_code)
            $scopeMap = [
                'branch_ids' => 'COALESCE(b_direct.id, b_via_branch.id)',
                'province_ids' => 'COALESCE(pv_direct.id, pv_via_branch.id)',
                'area_ids' => 'COALESCE(ar_direct.id, ar_via_province.id)',
                'head_office_ids' => 'COALESCE(ho_direct.id, ho_via_area.id)',
            ];
            ...
            // COMMENT: ไม่ต้องกรอง department_code แล้ว เพราะให้ดู version_plan_id เป็นหลัก
            // ถ้า request เลือก version_plan = จังหวัด แม้สร้างโดยสาขา ก็จะไปแสดงที่จังหวัด
            // $userDepartCode = $approvalScope['user_depart_code'] ?? null;
            // $userLevel = $approvalScope['user_level'] ?? null;
            // if ($userDepartCode && $userLevel && $userLevel !== 'head_office') {
            //     if ($userLevel === 'branch') {
            //         $param_count++;
            //         $where_conditions[] = "d.depart_code = $" . $param_count;
            //         $params[] = $userDepartCode;
            //     }
            // }

            // LOGIC ใหม่: ไม่กรอง version_plan_id แล้ว ให้ดูที่ stage (status) แทน
            // เพราะ version_plan กำหนดว่าเริ่มจาก stage ไหน:
            // - version_plan_id = 1 (สาขา) → เริ่มที่ pending_branch
            // - version_plan_id = 2 (จังหวัด) → เริ่มที่ pending_province (ข้ามสาขา)
            // - version_plan_id = 3 (เขต) → เริ่มที่ pending_area
            // - version_plan_id = 4 (สนง.ใหญ่) → เริ่มที่ pending_head_office
            //
            // User จังหวัดจะเห็น requests ที่:
            // - version_plan_id = 1 และ status = pending_province (วิ่งผ่านจังหวัด)
            // - version_plan_id = 2 และ status = pending_province (เริ่มที่จังหวัด)
            // ดังนั้น ไม่ต้องกรอง version_plan_id แต่กรองด้วย status (visible_statuses) แทน
            //
            // FK chain (branch_ids, province_ids) จะจัดการว่าเห็น requests จากสาขา/จังหวัดไหน

            // COMMENT: ลบการกรอง version_plan_id ออก
            // if ($userLevel) {
            //     $userVersionPlanId = $userLevelToVersionPlanId[$userLevel] ?? null;
            //     if ($userVersionPlanId) {
            //         $param_count++;
            //         $where_conditions[] = "r.version_plan_id = $" . $param_count;
            //         $params[] = $userVersionPlanId;
            //     }
            // }
        }
      ...
      // error_log('📋 Parameters: ' . json_encode($params));
        ...
        $selectParts = [
            ...
            "CONCAT_WS(' ', bs.code, bs.name) AS budget_source_name_full",
            'es.name as asset_nature_name',
            'ac.name as asset_category_name',
            ...
            'd.depart_code as department_code',
            'd.depart_name as department_name',
            // ใช้ COALESCE เพื่อเลือก branch/province/area/head_office ที่ถูกต้องตาม depart_code
            'COALESCE(b_direct.id, b_via_branch.id) as branch_id',
            'COALESCE(b_direct.branch_code, b_via_branch.branch_code) as branch_code',
            'COALESCE(b_direct.branch_name, b_via_branch.branch_name) as branch_name',
            'COALESCE(pv_direct.id, pv_via_branch.id) as province_id',
            ...
            'COALESCE(ho_direct.id, ho_via_area.id) as head_office_id',
            'COALESCE(ho_direct.head_office_code, ho_via_area.head_office_code) as head_office_code',
            'COALESCE(ho_direct.head_office_name, ho_via_area.head_office_name) as head_office_name'
        ];
        ...
        $joinParts = [
            ...
            // วิธีใหม่: JOIN แยก 2 ทาง - ทาง branch (via user.branch_id) และทางตรง (via depart_code)
            // แล้วใช้ COALESCE เลือกค่าที่ไม่ NULL

            // ทาง 1: JOIN ผ่าน user.branch_id (สำหรับ user ที่เป็นสาขา)
            "LEFT JOIN users.tb_branchs b_via_branch ON b_via_branch.id = u.branch_id",
            "LEFT JOIN users.tb_provinces pv_via_branch ON pv_via_branch.id = b_via_branch.province_id",
            "LEFT JOIN users.tb_areas ar_via_province ON ar_via_province.id = pv_via_branch.area_id",
            "LEFT JOIN users.tb_head_offices ho_via_area ON ho_via_area.id = ar_via_province.head_office_id",

            // ทาง 2: JOIN ตรงจาก depart_code (สำหรับ user ที่เป็นจังหวัด/เขต/สนง.ใหญ่)
            "LEFT JOIN users.tb_branchs b_direct ON b_direct.branch_code = d.depart_code",
            "LEFT JOIN users.tb_provinces pv_direct ON pv_direct.province_code = d.depart_code",
            "LEFT JOIN users.tb_areas ar_direct ON ar_direct.area_code = d.depart_code",
            "LEFT JOIN users.tb_head_offices ho_direct ON ho_direct.head_office_code = d.depart_code"
        ...
        if ($this->ensureReportFieldSupport()) {
            ...
            if ($this->hasReportFieldColumn('equipment_code_type_id')) {
                ...
                $joinParts[] = "LEFT JOIN parameters.tb_equipment_code_type ect ON r.equipment_code_type_id = ect.id";
            } else {
                $selectParts = array_merge($selectParts, [
                    'NULL::INTEGER AS equipment_code_type_id',
                    'NULL::VARCHAR AS equipment_code_type_code',
            ...
            if ($this->hasReportFieldColumn('equipment_type_id')) {
                ...
                $joinParts[] = "LEFT JOIN parameters.tb_equipment_type et ON r.equipment_type_id = et.id";
            } else {
                $selectParts = array_merge($selectParts, [
                    'NULL::INTEGER AS equipment_type_id',
                    'NULL::VARCHAR AS equipment_type_code',
            ...
            if ($this->hasReportFieldColumn('count_unit_id')) {
                ...
                $joinParts[] = "LEFT JOIN parameters.tb_count_units cu ON r.count_unit_id = cu.id";
            } else {
                $selectParts = array_merge($selectParts, [
                    'NULL::INTEGER AS count_unit_id',
                    'NULL::VARCHAR AS count_unit_name',
            ...
            // account_code
            if ($this->hasReportFieldColumn('account_code')) {
                $selectParts[] = 'r.account_code';
            } else {
                $selectParts[] = 'NULL::VARCHAR AS account_code';
            }
            ...
            if ($this->hasReportFieldColumn('fund_area_id')) {
                $selectParts = array_merge($selectParts, [
                    'r.fund_area_id',
                    'fa.code AS fund_area_code',
                    'fa.name AS fund_area_name'
                ]);
                $joinParts[] = "LEFT JOIN parameters.tb_ib_fund_area fa ON r.fund_area_id = fa.id";
            } else {
                $selectParts = array_merge($selectParts, [
                    'NULL::INTEGER AS fund_area_id',
                    'NULL::VARCHAR AS fund_area_code',
        ...
        if ($this->ensureBudgetFieldSupport()) {
            $selectParts = array_merge($selectParts, [
                'r.budget_approval_status',
                'r.budget_approved_amount',
                'r.budget_approval_remark',
                'r.budget_approved_by',
                'r.budget_approved_at',
                "CONCAT(u_budget.user_fname, ' ', u_budget.user_lname) as budget_approved_by_username"
            ]);
            $joinParts[] = "LEFT JOIN users.tb_users u_budget ON r.budget_approved_by::text = u_budget.id::text";
        } else {
            $selectParts = array_merge($selectParts, [
                "NULL::VARCHAR AS budget_approval_status",
                "NULL::NUMERIC AS budget_approved_amount",
                "NULL::TEXT AS budget_approval_remark",
                "NULL::INTEGER AS budget_approved_by",
                "NULL::TIMESTAMP AS budget_approved_at",
                "NULL::VARCHAR AS budget_approved_by_username"
            ]);
        }
     ...
     */
    public function updateStatus($id, $status, $updated_by, array $options = [])
    {
        $query = "UPDATE " . $this->table . " 
                  SET status = $1, updated_by = $2, updated_at = NOW()";

        $params = [$status, $updated_by];
        $paramIndex = 3;

        if (!empty($options['return']) && is_array($options['return'])) {
            $returnOptions = $options['return'];

            $query .= ", is_returned = $" . $paramIndex++;
            $params[] = !empty($returnOptions['is_returned']);

            $query .= ", returned_from_stage = $" . $paramIndex++;
            $params[] = $returnOptions['returned_from_stage'] ?? null;

            $query .= ", returned_reason = $" . $paramIndex++;
            $params[] = $returnOptions['returned_reason'] ?? null;

            $query .= ", returned_by_user_id = $" . $paramIndex++;
            $returnedBy = $returnOptions['returned_by_user_id'] ?? null;
            $params[] = $returnedBy !== null ? (int) $returnedBy : null;
        } elseif (!empty($options['clear_return'])) {
            $query .= ", is_returned = false, returned_from_stage = NULL, returned_reason = NULL, returned_by_user_id = NULL";
        }
