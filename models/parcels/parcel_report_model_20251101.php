...
class ParcelReportModel
{
    ...
    public function readPlanApproveReport($doc_type, $limit = 10, $offset = 0, $plan_number = null, $start_date = null, $end_date = null, $status_plan = null)
    {
        ...
        FROM parcels.tb_plans WHERE doc_type = $1 AND status_plan != 'delete' AND status_merge != 'none'";
        $params = [$doc_type];
    ...
    public function readWithdrawalSummaryReport($doc_type, $limit = 10, $offset = 0, $doc_number = null, $start_date = null, $end_date = null, $province_id = null)
    {
        // นับจำนวนทั้งหมดของข้อมูล
        $query_count = "SELECT COUNT(*) AS total FROM parcels.tb_docs WHERE doc_type = $1 AND status_doc != 'delete'";
        $params = [$doc_type];
        ...
        if (!empty($province_id)) {
            $query_count .= " AND province_id = $" . (count($params) + 1);
            $params[] = $province_id;
        }
        ...
        D.user_area,D.date_area,D.status_area,
        D.user_head_office,D.date_head_office,D.status_head_office,
        (select sum(quantity) from parcels.tb_doc_mats where doc_id = D.id) as quantity
        FROM parcels.tb_docs AS D
        LEFT JOIN (
            SELECT DISTINCT ON (doc_id) doc_id, quarter
            FROM parcels.tb_doc_mats
        ...
        if (!empty($province_id)) {
            $query .= " AND province_id = $" . (count($params) + 1);
            $params[] = $province_id;
        }
        if (!empty($doc_number) && !empty($province_id) && empty($start_date) && empty($end_date)) {
            $query .= " AND doc_number = $" . (count($params) + 1) . " AND province_id = $" . (count($params) + 2);
            $params[] = $doc_number;
            $params[] = $province_id;
        }
