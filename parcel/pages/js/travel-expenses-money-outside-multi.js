...
function setupApprovalUI(rootMain, isApprove, isAction) {
  ...
  if (rootMain.plan_id) {
    var userData = JSON.parse(sessionStorage.getItem("raot_user_session"));
    var permissionAPHO = userData.position_list_id.some(id => ['5'].includes(id));
    var permissionCHO = userData.position_list_id.some(id => ['12'].includes(id));
    var permissionAPAHO = userData.position_list_id.some(id => ['13'].includes(id));
    if (permissionAPHO) {
      if (approveStatus === "finance_check") {
        setApproveControls();
        $('.finance').show();
    ...
    var userData = JSON.parse(sessionStorage.getItem("raot_user_session"));
    var permissionAPHO = userData.position_list_id.some(id => ['5'].includes(id));
    var permissionAPHOP = userData.position_list_id.some(id => ['6'].includes(id));
    var permissionCHO = userData.position_list_id.some(id => ['12'].includes(id));
    var permissionAPAHO = userData.position_list_id.some(id => ['13'].includes(id));
    if (permissionAPHOP) {
...
function renderCurrentForm() {
  ...
  if (m.plan_id) {
    ...
    const firstChar = cp[0].wrd_mat_code.charAt(0);
    if (!isNaN(firstChar)) {
      $('#expense_cat').val(firstChar).trigger('change');
    }
  } else {
  ...
  document.getElementById("is_foreign").value = m.is_foreign || "";
  ...
  document.getElementById("expenses_other_remark").value = m.expenses_other_remark || "";
  ...
  if (!approvals.length) {
    const row = document.createElement("tr");
    row.innerHTML = `<td colspan="6" class="text-center text-danger">ไม่พบข้อมูล</td>`;
    ...
    approvals.forEach((item, index) => {
      ...
      row.innerHTML = `
                ...
                <td class="text-center">${item.approve_status === 'finance_approve' ? 'อนุมัติ' :
          item.approve_status === 'audit_approve' ? 'ผ่านการตรวจสอบแล้ว' :
            item.approve_status === 'finance_reject' ? 'ไม่อนุมัติ' :
              item.approve_status === 'audit_reject' ? 'ไม่ผ่านการตรวจสอบ' :
                ...
                <td class="text-center">${item.cat_text || ''}</td>
                <td>${item.remark || ""}</td>
            `;
      approvalTable?.appendChild(row);
    });
...
function bindApprovalControls() {
  ...
  if (btnSubmit) {
    btnSubmit.addEventListener("click", () =>
      updateData(approveCheckbox, rejectCheckbox, remarkTextarea, checkCheckbox, RejectCheckCheckbox, CheckAndApproveCheckbox)
    );
  }
}

async function updateData(approveCheckbox, rejectCheckbox, remarkTextarea, checkCheckbox, RejectCheckCheckbox, CheckAndApproveCheckbox) {
  try {
    const btnSubmit = document.getElementById("btn-update");
    ...
    if (!validateRejectRemark(rejectCheckbox, remarkTextarea, RejectCheckCheckbox)) return;
    if (!validateRejectExpenseCat()) return;
    ...
    const dataSent = {
      main_data: {
        ...
        check: checkCheckbox?.checked || false,
        reject_check: RejectCheckCheckbox?.checked || false,
        check_and_approve: CheckAndApproveCheckbox?.checked || false,
        user_code: state.dataUser?.user_code || "",
        cat_text: ($("#expense_cat option:selected").text() != 'เลือกค่าใช้จ่าย') ? $("#expense_cat option:selected").text() : null
      },
      ExpenseCat: $('#expense_cat').val()
    };
...
function validateRejectRemark(rejectCheckbox, remarkTextarea, RejectCheckCheckbox) {
...
function validateRejectExpenseCat() {
  const ExpenseCat = $('#expense_cat').val();
  if (state.posCode == 'APHO') {
    if (!ExpenseCat) {
      Swal.fire({
        title: "ข้อมูลไม่ครบถ้วน",
        text: "กรุณาเลือกจำแนกค่าใช้จ่าย",
        icon: "warning"
      });
      return false;
    }
