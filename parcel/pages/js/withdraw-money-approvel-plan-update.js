...
(async () => {
    try {
        ...
        const MatchApprove = userData?.position_list_id
            .some(id => ['5'].includes(id));
        const MatchCheck = userData?.position_list_id
            .some(id => ['12'].includes(id));
        const MatchApproveAndCheck = userData?.position_list_id
            .some(id => ['13'].includes(id));
        // remarkTextarea.disabled = true;

        if (MatchApprove) {
            $('.div_approve').css('display', '');
            $('.div_reject').css('display', '');
        }
        if (MatchCheck) {
            $('.div_check').css('display', '');
            $('.div_reject_check').css('display', '');
        }
        if (MatchApproveAndCheck) {
            $('.div_check_and_approve').css('display', '');
        }
        ...
        try {
            ...
            if (data.main_data.st_hf === 'finance_check') {
                $('.div_check').css('display', 'none');
                $('.div_reject_check').css('display', 'none');
                $('.div_check_and_approve').css('display', 'none');
                $('.div_approve').css('display', '');
                $('.div_reject').css('display', '');
            }
