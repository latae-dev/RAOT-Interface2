...
async function getPlanOne(id) {
    try {
        ...
        addDataPlan(data.tb_docs,data.tb_doc_mats[0]);

        quarter = data.tb_doc_mats[0]['quarter']
        doc_type = data.tb_docs.doc_type
...
function addNewRow(item = {}, matlistsSelectId = "", countUnitSelectId = "") {
    ...
    <td>
        <input disabled name="quantity_max" class="form-control text-center quarter" type="text"
            value="${parseFloat(item.quantity_max)-parseFloat(item.quantity) !== undefined ? parseFloat(item.quantity_max)-parseFloat(item.quantity) : 0}" 
    ...
    <td><input name="deli_date" class="form-control text-center" type="date" value="${item.deli_date}"></td>
    ...
    <td style="display: none;"><input name="id" class="form-control text-center" type="text" value="${item.id}"></td>
    `;
...
const deletedIds = []; // ตัวแปรเก็บ id ที่ถูกลบ
function deleteSelectedRows() {
    document.querySelectorAll(".rowCheck:checked").forEach(checkbox => {
        const row = checkbox.closest("tr");
        const idInput = row.querySelector('input[name="id"]');
        if (idInput) {
            deletedIds.push(idInput.value); // เก็บ id ไว้ใน array
        }
        row.remove();
    });
    ...
    updateRowIndexes(); // ฟังก์ชันนี้คุณมีอยู่แล้ว
    console.log("IDs ที่ถูกลบ:", deletedIds); // สำหรับตรวจสอบ
}

function addDataPlan(item,tb_doc_mats) {
    ...
    document.querySelector('input[name="quarter"]').value = tb_doc_mats.quarter;
}
...
async function updateData() {
    ...
    try {
        window.showLoading();
        const response = await fetch(`../controllers/parcels/doc_controller.php?action=update_doc`, {
            method: 'PUT',
