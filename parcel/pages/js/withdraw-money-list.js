  ...
  try {
    ...
    function displayData(data, pageOffset = 0) {
      ...
      if (!data || data.length === 0) {
        ...
        row.innerHTML = '<td colspan="7" class="text-center">ไม่พบข้อมูล</td>';
      ...
      data.forEach((item, index) => {
        const statusLabels = {
          waiting: `<span class="badge rounded-pill bg-warning">รอการตรวจสอบ</span>`,
        ...
        if (item.st_hf != "waiting" && item.st_hf != "reject" && item.st_hf != 'cho_reject') {
        ...
        const canEditOrDelete =
          ...
          (item.st_hf === "waiting" || item.st_hf === "reject" || item.st_hf === 'cho_reject');
    ...
    function resetSearchFields() {
      ...
      window.choicesInstance.setChoiceByValue(""); // เลือก placeholder
    }
