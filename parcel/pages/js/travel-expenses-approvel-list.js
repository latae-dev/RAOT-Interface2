      ...
      data.forEach((item, index) => {
        ...
        if (!item.plan_id) {
          if (item.req_user_code != params_all.current_user.user_code) {
            ...
            if (permissionAPHOP) {
              if (item.approve_status === "waiting") {
                approveButton = `
              ...
              if (item.approve_status === "finance_check") {
                approveButton = `
                        <a href="${linkView}?id=${item.id}&is_approve=1&is_action=approve" 
                        class="btn btn-icon btn-sm btn-warning-transparent rounded-pill" 
                        title="การพิจารณาอนุมัติ">
          ...
          if (item.req_user_code != params_all.current_user.user_code) {
            ...
            } else if (permissionAPAHO && (item.approve_status === "waiting" || item.approve_status === "finance_check")) {
              approveButton = `
                        <a href="${linkView}?id=${item.id}&is_approve=1&is_action=approve" 
                        class="btn btn-icon btn-sm btn-warning-transparent rounded-pill" 
                        title="การพิจารณาอนุมัติ">
