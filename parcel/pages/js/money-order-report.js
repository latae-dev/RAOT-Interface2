...
(async () => {
    try {
        ...
        $.ajax({
            type: "GET",
            url: "../controllers/withdraws/wrd_compensation_controller.php?action=get_persons_report&depart_code=" + params_all.current_user.depart_code,
            dataType: "json",
        ...
        function displayData(data, pageOffset = 0) {
            ...
            if (!data || data.length === 0) {
                ...
                row.innerHTML = '<td colspan="12" class="text-center">ไม่พบข้อมูล</td>';
            ...
            data.forEach((item, index) => {
                ...
                row.innerHTML = `
                    <td>${convertToBuddhistEra(item.start_date)}</td>
                    <td style="text-align:left;">${item.full_name}</td>
                    <td style="text-align:center;">${item.affiliation_name}</td>
                    <td style="text-align:left;">${item.compenstaion_items}</td>
                    <td>${(item.compensation_amount && item.compensation_amount != 0) ? Number(item.compensation_amount).toLocaleString(undefined, { minimumFractionDigits: 2 }) : '-'}</td>
...
document.getElementById("exportExcelBtn").addEventListener("click", function () {
    ...
    XLSX.writeFile(wb, "รายงานใบสั่งจ่าย.xlsx");
});
