...
function radioMark($checked, $label)
{
    return '<span style="
      ...
      font-size:16px;
      margin-right:3px;
    "><label style="font-size:17pt;line-height:12px!important;margin-left:-1px">'
        . ($checked ? '●' : '&nbsp;') .
        '</label></span> ' . $label;
}
// -------------------- HTML หน้า 1 --------------------
$html1 = '';
$count_page_detail = ceil(count($detail_data) / 6);
$data_item = 0;
$total_bath = 0;
$total_stang = 0;
$total_all = 0;
for ($pages = 0; $pages < $count_page_detail; $pages++) {
    $item = 0;
    $html1 .= '
...
<table border="1" style="border:1px solid #000;margin-top:10px;">
    ...
    // แสดงหน้าละ 6 รายการ
    for ($i = 0; $i < 6; $i++) {
        if ($data_item < count($detail_data)) {
            $html1 .= '<tr>
                <td class="hide-top-bottom" style="text-align:center;">' . ($data_item + 1) . '</td>
                <td class="hide-top-bottom">' . $detail_data[$data_item]['detail'] . '</td>
                <td class="hide-top-bottom" style="text-align:center;">' . number_format($detail_data[$data_item]['bath']) . '</td>
                <td class="hide-top-bottom" style="text-align:center;">' . $detail_data[$data_item]['stang'] . '</td>
            </tr>';
            $total_bath += $detail_data[$data_item]['bath'];
            $total_stang += $detail_data[$data_item]['stang'];
            if ($total_stang >= 100) {
                $total_bath += floor($total_stang / 100);
                $total_stang = $total_stang % 100;
            }
            $total_all += $detail_data[$data_item]['bath'] + ($detail_data[$data_item]['stang'] / 100);
            $data_item++;
        } else {
            $html1 .= '<tr>
    ...
    <tbody>
        <tr>
            <td colspan="2" style="position: relative;">
                <div style="float:left;">' . ($pages == $count_page_detail - 1 ? '&nbsp;&nbsp;(๙) (ตัวอักษร) ' . convert_number_to_text($total_all) : '&nbsp;') . '</div>
                <div style="float:right;">' . ($pages == $count_page_detail - 1 ? 'รวม&nbsp;&nbsp;' : '') . '</div>
            </td>
            <td style="text-align:center;">' . ($pages == $count_page_detail - 1 ? number_format($total_bath) : '&nbsp;') . '</td>
            <td style="text-align:center;">' . ($pages == $count_page_detail - 1 ? $total_stang : '&nbsp;') . '</td>
        </tr>
        <tr>
...
<p style="text-align:right;' . ($pages == $count_page_detail - 1 ? '' : 'page-break-after:always;') . '">กยท.๙๑-๐๑-๕๓/๖๕</p>
';
}
// -------------------- HTML หน้า 2   --------------------
$html2 = '
