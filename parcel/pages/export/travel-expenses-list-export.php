...
if ($main['expenses_group'] == 3 && $main['plan_id']) {
  $plan_total = arabicToThaiNumber($main['plan_total']);
  $expenses_amount = arabicToThaiNumber(number_format((float)$main['expenses_amount'], 2));
  $family = $expensesModel->readExpensesFamily($main['id']);
  $total_family = 0;
  foreach ($family['records'] as $rs) {
    $total_family += $rs['expenses_total'];
  }
  $diff = floatval(str_replace(',', '', $main['plan_total'])) - $total_family;
  if ($diff < 0) {
    $expenses_action = 'withdraw';
    $expenses_amount = arabicToThaiNumber(number_format(abs($diff), 2));
  } else {
    $expenses_action = 'return';
    $expenses_amount = arabicToThaiNumber(number_format($diff, 2));
  }
} else if ($main['expenses_group'] == 3 && !$main['plan_id']) {
  $plan_total = arabicToThaiNumber($main['plan_total']);
  $expenses_amount = arabicToThaiNumber(number_format((float)$main['expenses_amount'], 2));
  $family = $expensesModel->readExpensesFamily($main['id']);
  $total_family = 0;
...
ประกอบใบเบิกค่าใช้จ่ายในการเดินทาง' . dottedLine("", 180, 'center', 'dotted') . ' สังกัด ' . dottedLine("", 180, 'center', 'dotted') . ' ลงวันที่ ' . dottedLine("", 70, 'center', 'dotted') . ' เดือน ' . dottedLine("" . "", 150, 'center', 'dotted') . ' พ.ศ. ' . dottedLine("", 70, 'center', 'dotted') . '
</p>
...
<table border="1">
  <tr class="center bold">
    ...
    <th class="center-middle" style="font-size:14pt" rowspan="2" width="10%">ตำแหน่ง/ระดับ</th>
