...
class WrdExpensesApproveHistoryModel
{
    ...
    public function createApproveLogs($data, $id, $position_code, $info)
    {
        ...
        if (!empty($data['main_data']['approve']) || !empty($data['main_data']['check_and_approve'])) {
            $cat_text    = $data['main_data']['cat_text'] ?? null;
        } else {
            $cat_text = null;
        }
        ...
        if (!empty($data['main_data']['approve'])) {
            ...
            if ($info['approve_status'] != 'finance_check') {
                $checkQuery = "INSERT INTO withdraws.tb_wrd_expenses_approve_history (
                expenses_id,
                user_code,
                approve_date,
                approve_status,
    ...
    public function createApproveLogsMulti($data, $id, $position_code, $info)
    {
        $map = [
            'APHO' => [
                'approve' => 'finance_approve',
