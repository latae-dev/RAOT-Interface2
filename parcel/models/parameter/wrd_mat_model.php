...
class WrdMatModel
{
    ...
    public function readAll()
    {
        ...
        $query = "SELECT 
        WR.code AS mat_code, WR.name AS mat_name,WR.no AS mat_no,WR.id AS mat_id,
        WG.no AS group_no, WG.name AS group_name
    ...
    public function readType($types)
    {
        /* $query = "
            SELECT 
                WM.*, 
        ...
        FROM parameters.tb_wrd_mats AS WR 
        LEFT JOIN parameters.tb_wrd_groups AS WG ON WG.id=WR.wrd_group_id
        where WR.wrd_group_id IN (" . implode(',', array_map('intval', $types)) . ")";
